...
#### `_thesis-manifest (thesis-TICKER-*).md`
Two-phase write matching the other manifests. `/thesis` Step 3.5 writes skeleton before any file modification; records intended thesis file path, sector note edit plan, `_hot.md` edits, orphan research integration targets, and (if encountered) the archive-collision decision from Step 1.2. Step 7.5 flips status. `/rollback` Step 2.5f cascade offers (a) delete thesis file only, (b) full cascade (delete + revert sector + revert `_hot.md` + orphan mtime revert if captured), or (c) cancel. Unlike status/sync which restore FROM snapshots, thesis cascade is deletion-based since `/thesis` creates new files with no "before" state to snapshot. `/lint #49` handles aging + in-progress detection. No regret-recovery window — `/thesis` is constructive, not destructive; post-hoc edits are `/deepen`'s job.
...
| `/lint #49` — thesis manifest `status: in-progress` | `/thesis` Step 7.5 flip failed OR `/thesis` crashed mid-run | If thesis file exists AND all listed sections landed (sector, `_hot.md`), edit manifest to `status: completed`. If partial, either manually complete remaining steps OR `/rollback [batch]` → option (b) full cascade (deletes thesis + reverts sector/hot.md). |
| `/rollback` on a thesis batch ID offers "delete thesis only" vs "full cascade" | Step 2.5f cascade — `/thesis` creates files rather than modifying (no pre-state to restore) | (a) Delete thesis file only — leaves sector + `_hot.md` edits dangling (manual cleanup). (b) Full cascade — deletes thesis + reverts all associated edits. Consider saving thesis content elsewhere first if (b). |
| `/catalyst` aborts with "pre-regenerate snapshot failed" | Disk full, permission, or `_Archive/Snapshots/` unwritable | Prior `_catalyst.md` is unchanged. Fix filesystem issue and re-run. Hard-abort guarantees the prior calendar is never lost to a mid-regenerate failure. |
| `/rollback` neighbor scan flags macro or sector citations | Step 6.2.5 extended scan (H3) — macros and sectors may cite the restored thesis in body prose outside Log sections | Review each citation for post-closure premise. Macros with prose like "LITE's closure removes..." need manual edit. Sector narrative (Acquisitions, Industry history) may need update. Use the classification tag (premise-dependent vs contextual) in the scan report to prioritize. |
| `/sync` propagates a research note twice after renaming it | Pre-H4: wikilink-only idempotency broke on rename. **Fixed (H4)**: secondary keys (source URL, date+ticker, tags) preserve idempotency across renames/moves | If legacy duplicates exist: manually consolidate via strikethrough. New runs handle renames correctly. |
