...
Per AGENTS.md ("Read the Mental Models folder before any investment analysis"), the gate applies to **two** classes of skill, not just section-writers:

1. **Section writers** — `$sync`, `$deepen`, `$ingest` (identifier) — must read before writing `## Mental Models`.
2. **Judgement-renderers (fresh analysis)** — `$thesis`, `$stress-test`, `$compare`, `$scenario`, `$surface`, `$retro`, `$prune`, `$deepen`, `$status`, `$transcript` — must read the `/Mental Models` folder before producing ANY investment judgement (a new thesis, an adversarial case, a comparison, a scenario impact, an opportunity ranking, a conviction/status read, a section deepen, an earnings-signal-vs-thesis cross-reference). `$deepen` is BOTH a section-writer (class 1) and a judgement-renderer: a deepen of Bull Case / Risks / Industry Context renders judgement even when it never touches `## Mental Models` — AGENTS.md's mandate explicitly enumerates "a deepen", so the gate applies to every deepen run, not only Mental-Models-writing ones. **`$status` and `$transcript` were added 2026-07-15** (CHG-12): AGENTS.md's mandate enumerates "a conviction/status judgement", and `$status` executes exactly that, yet was omitted from this list — a silent gate leak; `$transcript` Step 6 cross-references earnings signals against the thesis's Bull/Bear/Triggers, which is judgement-rendering. Both read the load-tiered scope (Generalist always; the matching Industry/Lens by ticker), cached across the run. This is the deepest analytical work in the vault and AGENTS.md's mandate governs it directly — even when the skill does not itself write the `## Mental Models` section.
3. **Derivative distiller** — `$brief` — does NOT re-read the folder (it distills an existing thesis, it does not analyze anew, and it changes no conviction/status; AGENTS.md's enumerated mandate does not list it). Its gate is satisfied by **consuming the thesis's already-applied `## Mental Models` section and `## Conviction Triggers`** — specifically, the Non-Consensus Edge must name its falsifier from the thesis's `→ LOW if` / `→ CLOSE if` trigger. Re-reading the folder for a distillation would be redundant.
...
**Delegated skills (`$surface`, `$retro`, `$prune`).** A `general-purpose` subagent does NOT inherit AGENTS.md, so the mandate reaches it only if the delegated prompt carries it. Each delegated prompt MUST embed this gate verbatim (with absolute `Mental Models/` paths) — the load tiering (Generalist always; Industry/Lens by scope) and the READING PROTOCOL. A delegated prompt that omits the gate is a spec violation: the mandate silently evaporates at the delegation boundary.

**Load tiering (mechanical/metadata skills are exempt).** The gate governs *analytical* work. `$numbers`, `$rename`, `$lint`, `$clean`, `$graph`, `$catalyst`, `$archive-callouts`, `$rollback` produce no investment judgement and skip the gate entirely (matches AGENTS.md "Load tiering"). Scope the conditional files to the task — load `Industry - Semiconductors` only for semi names, the two `Lens` files only when that lens is in play; do not bulk-load all four for a single-name question they do not touch.
...
2. **Selectivity — same bar as the rest of the note.** Update ONLY when a model trigger is newly activated, retired, or its read materially changed. Do NOT restate triggers already present; do NOT add one on trivial delta. **An unchanged section is the correct outcome for most runs** (mirrors `$sync` Step 3b "Do NOT update sections where the delta is trivial").
   - **Exception — empty scaffold with qualifying content (CHG-14).** Selectivity governs *populated* sections. If the `## Mental Models` section is still the unpopulated template scaffold (only the three `<!-- … -->` placeholder bullets) AND the run has qualifying content on the page — a fired model trigger this run, OR existing body content that plainly activates a model (e.g. a sector note documenting a named layer-monopoly should fire Value Layer Monopoly §1) — then populating it is **NOT optional**. An empty scaffold sitting next to qualifying content is the one case selectivity does not excuse: the ABF Substrates sector note sat scaffold-empty through 5 consecutive `$sync` runs exactly because each run read "empty section, trivial delta → skip." First-population is Tier B (additive, no snapshot); it is not churn.
