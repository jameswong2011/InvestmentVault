...
> **Design note**: Treat Codex shell calls as stateless unless the runtime explicitly continues the same shell session — shell state (including `trap` handlers) does NOT persist across separate calls. Each independent shell block may run in a fresh subshell. The lock scheme must therefore be stateless at the shell level and durable across multiple tool calls (shell / file-read / patch / file-write) within a single skill run. The LLM running the skill carries lock identity in its conversation context between tool calls.
...
session_id: <codex-thread-id or "unknown">
---
...
### 1.3 Acquisition (Step 0.1 — single shell block)

Step 0.1 emits one shell block that:
...
# subshell only; does not leak to any other skill's shell blocks.
...
When Step 0.1 shell echoes `LOCK_HELD|...`:
...
When Step 0.1 shell echoes `STALE_LOCK|...`:
...
### 1.5 Ownership verification (mandatory at start of every subsequent shell block)

After Step 0.1 succeeds with `ACQUIRED|...`, the LLM has captured `$TOKEN`. Every subsequent shell block in the skill MUST begin with:
...
Between-block tool calls (Read / Edit / Write without Bash) do NOT need verification — they cannot release or overwrite the lockfile. The lock persists until either (a) an explicit release in the skill's final shell block, (b) a user `rm`, or (c) the INT/TERM trap fires on signal.
...
### 1.7 Release (explicit final shell block)

The skill's FINAL shell block (typically in the reporting phase) runs:
...
The release shell block is unconditional — it runs whether the skill succeeded or hit a non-fatal error. If the skill aborts before reaching the release step (crash, hard abort on section missing, etc.), the lock persists and appears stale on the next collision / `$lint` run.

Trap on INT/TERM handles Ctrl-C and kill signals within the acquisition shell block only — subsequent blocks don't inherit the trap (by design; the LLM's explicit final-release block is the primary cleanup path).
...
For `$compare` (multi-ticker), the check runs for each ticker in `SKILL_ARGS`; the skill aborts on the FIRST marker found.
