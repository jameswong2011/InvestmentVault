...
```
Reclaim a stale lock (`STALE_LOCK`) only per §1.6 — never auto-steal. Release in the final shell block with the ownership-verified form (`grep '^token:' .vault-lock` == `$TOKEN` before `rm -f`).
...
```bash
python3 .agents/skills/graph/scripts/generate_graph.py full   # or: last
```
...
The generator already self-validated. Confirm structure and release the lock in one shell block:
...
1. **Extract** inventory + reverse indexes + per-thesis wikilinks/status/log_tail in bounded shell blocks — batch theses ≤20 per block (the shell tool truncates output ~20 KB).
