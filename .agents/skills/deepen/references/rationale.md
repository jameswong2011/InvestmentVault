...
`deepen-TICKER-YYYY-MM-DD-HHMMSS` — TICKER embedded to prevent collisions between concurrent `$deepen` runs on different tickers that hit the same HHMMSS. Matches `$stress-test` pattern (§3 of `.agents/skills/stress-test/references/rationale.md`).
...
Registry entries: `.agents/skills/_shared/log-prefixes.md` §2 (Deepening provisional), §3 (Deepened), §4 (↳ CORRECTION: Deepened). Changing any of these three prefixes without updating the registry and `$sync` breaks drift detection silently. `$lint #29` verifies registry/producer/consumer alignment.
