...
**Codex execution:** Read `.agents/skills/_shared/codex-compat.md` first. Treat `SKILL_ARGS` as the arguments supplied with `$prune`, or infer them from the user's request when this skill is invoked implicitly.
...
Design rationale in `.agents/skills/prune/references/rationale.md` (§N.M anchors).
...
Parse `SKILL_ARGS`. Arguments can combine (e.g., `$prune semiconductors stale`):
...
Use `read-only` scope for the delegated analysis and `vault-wide` scope for Phase 5, per `.agents/skills/_shared/preflight.md` Procedure 1. Timeout 15 min (large portfolios are slow). Capture each token, verify ownership (Procedure 1.5) every block, and release it at the end of its stage.
...
**Thesis snapshots** — one per approved closure AND per approved upgrade. Issue ALL `cp` calls in ONE shell block using background shell jobs + `wait` (mirrors `$compare` Phase 5.5a and `$rollback` Step 4 patterns — a 5-closure batch is 5 sequential `cp` rounds without this; one round-trip with it):
...
**Sector note snapshots** — use the `(sector_note_path, match_confidence, log_message)` tuples resolved in Stage 1. Dedup, then snapshot each unique sector note in the SAME background-jobs shell block as the thesis snapshots above (one `wait` for all snapshots across both categories). Frontmatter (`snapshot_trigger: prune`, same batch ID).
...
- **Max 2 lines per AGENTS.md Writing Standards**. Collapse long lists: `... and 2 more — see $prune output`.
...
2. **Verify flip landed** (targeted live-file verification): run a targeted frontmatter read against the live file after applying the patch. Do not rely only on the patch operation's success status. Confirm `status: completed` present, `status: in-progress` absent from the targeted verification output.
   - **Success** → manifest stays in `_Archive/Snapshots/` with `status: completed`. `$clean` + `$lint #36` handle aging.
   - **Verification failure** (§3.4 — targeted verification indicates replacement did not produce expected frontmatter, or patch operation returned an error): do NOT attempt repair. Report:
     ```
