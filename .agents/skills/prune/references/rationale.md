...
Log appends are append-only per AGENTS.md Change Safety Rules. Snapshotting every affected neighbor would explode the batch size — a thesis closure with 20 cross-references would create 20 additional snapshots. The append operation is atomic Edit; if it fails, the thesis is unchanged (no partial state).
...
Rationale: max 2 lines per AGENTS.md Writing Standards. One entry per neighbor regardless of how many closures affected it. Collapse rationales if the list runs long (`... and 2 more — see $prune output`).
