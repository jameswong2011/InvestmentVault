...
AGENTS.md Change Safety Rule #2 (Tier 3) classifies `_Archive/` content as archival record — never modified without explicit instruction. A thesis that received a `Scenario` Log entry and was subsequently closed/archived has its Log preserved as historical audit trail.
...
AGENTS.md Tier 2 rule "research notes are immutable source records" applies — reverse mode signals "no longer applies" without erasing the original analysis.
