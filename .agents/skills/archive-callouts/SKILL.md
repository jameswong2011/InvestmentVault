...
**Codex execution:** Read `.agents/skills/_shared/codex-compat.md` first. Treat `SKILL_ARGS` as the arguments supplied with `$archive-callouts`, or infer them from the user's request when this skill is invoked implicitly.
...
**Tier 1 skill under AGENTS.md Change Safety Rules** — owns the `## Legacy Callouts` section exclusively. Nothing else writes to it; this skill never writes anywhere else except the per-thesis `## Log` (append-only `Callout sweep:` entry) and thesis frontmatter (`last_callout_sweep:`).
...
Parse order:
```
Split SKILL_ARGS by whitespace into tokens.
...
Per `.agents/skills/_shared/preflight.md` Procedure 1.
...
Capture the token. Verify ownership (Procedure 1.5) at every subsequent shell block. Release in the final reporting block.

### 0.2: Rename-marker check

Per `.agents/skills/_shared/preflight.md` Procedure 2.
...
- **Vault-wide**: `Theses/*.md` + `Sectors/*.md` + `Macro & Technology/*.md`. Research/ is excluded (Tier 2 immutable per AGENTS.md — research notes don't carry callouts by convention).
...
Note: `error` maps to `warning` because the template file is `user-warning.md` (user-facing vocabulary matches the hotkey label, not the Obsidian internal type). Consistent with AGENTS.md convention.
...
Sub-bullet uses 2-space indent (matching AGENTS.md markdown convention).
...
For each file with ≥1 candidate, issue ONE shell block with parallel `cp` + `wait`:
...
Log prefix `Callout sweep:` is registered in `.agents/skills/_shared/log-prefixes.md` entry #17 as skill-origin (drift-exclusion, propagation-skip per `$sync` Step 2.5 and Step 3e).
...
For each swept file, run ONE shell probe:
...
8. **Log prefix is `Callout sweep:`.** Registered in `.agents/skills/_shared/log-prefixes.md` §17 as skill-origin drift-exclusion. Producer drift here breaks `$sync` Step 2.5 and Step 3e silently.
...
Design rationale lives in `.agents/skills/archive-callouts/references/rationale.md`:
