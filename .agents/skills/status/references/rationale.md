...
`$sync` Step 3e drift detection uses `"Conviction reaffirmed"` as the window anchor. Registry entry: `.agents/skills/_shared/log-prefixes.md §5`. Changing the prefix silently breaks drift detection's anchor logic — drift windows would count entries from before the reaffirm, producing false positives.
...
AGENTS.md Tier 3's stated rule: "These changes represent investment decisions, not formatting — confirm with the user before applying". The listed examples are:
...
- **`active→draft`**: a demotion — explicitly Tier 3 per AGENTS.md.
...
|---|---|---|
| `$sync` Step 2.5 skill-origin classification | Reads Log prefix `"Status change:"` from `.agents/skills/_shared/log-prefixes.md` | **Zero** — prefix written identically by Step 2F and Step 2 |
...
| AGENTS.md Tier 3 rule | Lists examples of confirmation-required transitions | **Compatible** — examples are reductions; draft→active is additive and isn't listed |
...
Step 7.9 manifest verification re-read can dispatch in parallel with Step 8's final `rm -f $LOCK_FILE` shell block — different files, no dependency. One more round-trip saved.
