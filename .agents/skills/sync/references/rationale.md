...
Research-note-path keying (wikilink presence anywhere in the Log) eliminates this: once a research note has propagated to a thesis, it is terminal — the thesis's Log is a permanent audit of which research notes have informed it. This also matches the semantic meaning of AGENTS.md Tier 2 append-only: research notes are immutable source records; after initial propagation, subsequent runs on the same note produce no new signal.
...
If the user edits a research note's body later and wants to re-propagate, the contract is: create a NEW research note OR append a thesis Log entry directly (AGENTS.md Tier 2 "research notes are immutable source records"). `$sync` does not re-propagate a research note that has already reached a thesis, regardless of when or how the research note changed.
...
Both scans did the same work against the same data. The T7.1 consolidation runs ONE vault-wide `Grep` per research note across `Theses/*.md` (using the five wikilink forms from `.agents/skills/_shared/wikilink-forms.md`). The result set is used for both:
...
1. **Skill content was truncated in the initial context.** The system-reminder delivered Steps 0 through 2.9 of SKILL.md inline and then marked `[... skill content truncated for compaction ...]`. Step 4.-1 (the sector skill-origin gate) was NOT visible during execution. The LLM filled in Step 4 behavior from memory and general reasoning rather than explicit rules. This is the proximate cause. AGENTS.md Operational Rule #11 (added 2026-04-22) requires Reading the full SKILL.md before any write phase when truncation is detected.
...
Pass 1 read "only frontmatter + Summary + last 3 Log entries" per thesis. In practice, `Read` returns the whole file (file-reading tool semantics), and selective section reads require bash extraction that LLMs inconsistently apply.
...
Counting any of these would either inflate drift or consume a drift-window slot without signal. See `.agents/skills/_shared/log-prefixes.md` registry entries for exact producer/consumer linkage.
