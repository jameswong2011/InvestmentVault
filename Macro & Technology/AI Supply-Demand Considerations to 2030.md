...
Token supply capacity grows at roughly 3x per year through 2028: physical build (energised watts +40-50%/yr, HBM bit stock +55-60%/yr) multiplied by fleet-average efficiency (×1.7-2.0/yr on a measured basis, not vendor marketing), which we expect to fade toward ×1.4-1.7 in 2029-30 as the precision ladder that has supplied ~2x per generation runs out. The cost of that supply is flat per watt: ~$6.5 per watt-year on a 7-year accelerator and 25-year shell, ~$10.6 with the cost of capital charged, with electricity 4-12% of it. As such the industry's cost of producing tokens grows with energised watts rather than with tokens: total AI-fleet COGS ~$150B (2026) → ~$600B (2030) accounting and ~$240B → ~$950B economic, of which token serving is ~$75B → ~$420B, while tokens rise ×40 and cost per token falls ÷7 (Part I).

Token demand, compounding at 5-9x through mid-2026 and forecast here at ~3.5-4.5x (2027) fading to ~2.5-3.5x (2028) and ~2-2.5x after, runs above the supply line through 2027 and converges to it during 2028; tokens served follow the supply line, ×10 to end-2028 and ×40-50 by end-2030. Dollar demand is larger than the revenue line shows: compute expenditure runs ~$300-400B in 2026, 2-2.5x measured AI revenue, because training, hyperscaler internal workloads and open-weight serving all pay for watts without appearing as AI revenue. Every energised watt has a payer; the four payers differ in duration, not in whether they pay, and all of them scale with token revenue (Part II).

In tokens the market stays in managed famine through 2027, normalises rather than gluts in 2028, and carries rate discipline into 2029-30 on the power gate alone, with the glut window opening only if a financing discontinuity converts backlog into idle capacity. In dollars the compute layer earns ~1.4-1.7x its economic cost today, a famine multiple that normalises toward 1.0x as watts quadruple and expenditure grows ~×3-3.5 on the revenue bridge, without any deepening of the shortage; neocloud margins hold only if compute expenditure grows with total watts (×4 by 2030), so the base case lands new fleets at the 15% hurdle rather than today's spread and the famine persists only above the bridge. GPU-hour rates and token prices decouple through the efficiency engine (a flat contracted GPU-hour delivers a $/token falling 2-2.5x a year), which is the stable equilibrium the neocloud model requires; the famine premium on top of it is cyclical. Scarcity clears through a cascade (training reallocation, internal squeeze, queues, price discrimination, posted price last), so the blended price signal is damped while the marginal signals, the fast-tier/batch spread and the merchant GPU-hour, carry the information (Appendix A). We hold the balance of evidence at: demand at or above the balance line to late-2028 (~50%), extended famine (~25%), financing-led glut (~20-25%).

## Part I. Supply

### 1. Physical gates: watts and memory
...
### 2. The efficiency engine: tokens per watt
...
### 5. Dollar demand: who pays for the watts
...
### 6. Demand against supply: the regime path and the economic balance

Two balances run over the same window. In tokens, demand sits above the supply line through 2027 and converges during 2028; in dollars, compute expenditure sits above the cost of supply at a famine multiple that normalises as the fleet quadruples. Read together they give the regime path:
...
| Serving COGS per million tokens, accounting (§3) | $0.75 | $0.44 | $0.25 | $0.15 | $0.10 |
...
### B. Observables (ranked by information value)
...
### D. Method notes: cost basis, benchmarks and cross-checks
...
| All-in accounting, IT 5.5-yr / shell 17.5-yr | 7.5 | ~$500B | ~$720B |
| All-in accounting, IT 7-yr / shell 25-yr (used in §3) | 6.2 | ~$417B | ~$595B |
| All-in economic, IT at 15%/7-yr, shell at 10%/25-yr | 9.9 | ~$670B | ~$950B |

**Fleet path.** 19 GW installed at end-2025, derived from ~$1.0-1.2T of AI capital invested through 2026 at $35-47B/GW; year-end installed 30 / 44 / 60 / 82 / 110 GW for 2026-30 on the §1 additions, year-average 24 / 37 / 52 / 71 / 96 GW; inference share 50% → 70%. Compute expenditure in §5 is the same fleet at $14-17M per MW-year fleet-average rent ($340-400B), which the payer build reproduces at $285-400B.

**Benchmarks.** Rubin NVL72 on SemiAnalysis's renormalised CoreWeave data (DeepSeek R1, output tokens, all-in MW), converted at the §3 economic stack of $10.6M per MW-year:

| Interactivity (tok/s/user) | tok/s per MW | T tokens per MW-yr at 100% utilisation | $ per million output tokens | Realised (60% utilisation × 0.4 model/context) |
|---:|---:|---:|---:|---:|
| 50 | 1.33M | 42 | $0.25 | $1.05 |
| 135 | 886K | 28 | $0.38 | $1.6 |
| 300 | 96K | 3.0 | $3.5 | $15 |
| 350 | 71K | 2.2 | $4.8 | – |

**Cross-checks.** Big Four D&A of $170B rising to $260-300B by 2028, with ~60% of AI depreciation on inference, gives $200-250B of 2028 serving cost against the §3 $203B. About 1e17 tokens in 2026 at ~$0.75/M realised gives ~$75B. Epoch's GB200 1 GW build (5-yr IT life, 8.34c/kWh, PUE 1.14, 71% utilisation) annualises at $8.5M per MW-year with energy at 7% and servers plus network at 73%. The outside view is the IEA path (AI-focused electricity ×3 by 2030): ~40 GW of 2030 inference capacity, ~$250B serving, ~$370B fleet, the bottom of the ×3.3-5.5 range.
...
- Generalist [G-7] · ROIIC × runway, cost-side — tokens per capex dollar (~×15 by 2030) is the physical yield; revenue per capex dollar is the return, and the two diverge by the price-deflation rate. Hypothesis: the return variable for every layer is rent per watt, not tokens per watt (§3, §7).
