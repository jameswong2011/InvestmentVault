...
- **Semis #2 inverted — gate deletion** — China's repeated move is not crossing the qualification gate but choosing architectures that delete it (QDD vs flexspline, current-sensing vs F/T, vision vs LiDAR). Test for any "moat" claim in this chain: can an architecture change delete the gate entirely?
