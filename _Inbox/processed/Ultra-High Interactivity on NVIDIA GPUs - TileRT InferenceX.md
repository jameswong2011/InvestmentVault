...
- Pool A: Ultra high interactivity decode with TileRTLatency-critical requests pass through the TileRT PD Router, which instructs vLLM to generate the first token and marks the request with the destination TileRT node in kv_transfer_params.

- Pool B: General low to medium interactivity decode with vLLM decodeGeneral traffic continues through vLLM’s native disaggregation proxy to a conventional vLLM decode pool.
