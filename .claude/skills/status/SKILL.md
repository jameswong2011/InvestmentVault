...
1. **Identify the core assumption** the change rests on — from the user's rationale (`$ARGUMENTS`) plus the downgraded thesis's own `## Bull Case` / `## Key Non-consensus Insights` content (already in memory from the Step 1 read).
...
Runs for every conviction/status change; Reaffirm flow (Step 2R) exits before this step and never resolves findings. Closes the loop opened by writer skills per `.claude/skills/_shared/followups-contract.md`. **Non-fatal** — a failed read or edit never blocks the completed change.
