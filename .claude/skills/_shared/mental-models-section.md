...
2. **Judgement-renderers (fresh analysis)** — `/thesis`, `/stress-test`, `/compare`, `/scenario`, `/surface`, `/retro`, `/prune`, `/deepen`, `/status`, `/transcript` — must read the `/Mental Models` folder before producing ANY investment judgement (a new thesis, an adversarial case, a comparison, a scenario impact, an opportunity ranking, a conviction/status read, a section deepen, an earnings-signal-vs-thesis cross-reference). `/deepen` is BOTH a section-writer (class 1) and a judgement-renderer: a deepen of Bull Case / Risks / Industry Context renders judgement even when it never touches `## Mental Models` — CLAUDE.md's mandate explicitly enumerates "a deepen", so the gate applies to every deepen run, not only Mental-Models-writing ones. **`/status` and `/transcript` were added 2026-07-15** (CHG-12): CLAUDE.md's mandate enumerates "a conviction/status judgement", and `/status` executes exactly that, yet was omitted from this list — a silent gate leak; `/transcript` Step 6 cross-references earnings signals against the thesis's Bull/Bear/Triggers, which is judgement-rendering. Both read the load-tiered scope (Generalist always; the matching Industry/Lens by ticker), cached across the run. This is the deepest analytical work in the vault and CLAUDE.md's mandate governs it directly — even when the skill does not itself write the `## Mental Models` section.
