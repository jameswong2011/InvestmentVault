...
36. **Prune batch-manifest state** — Scan `_Archive/Snapshots/` for `_prune-manifest*.md` files. These are crash-recovery artifacts from `/prune` Stage 1.5; their presence indicates a prune operation state.
    ...
    - For each manifest found, read frontmatter and classify by `status:`:
      ...
      - **`status: completed`** (5.2 retention policy — 30-day regret-recovery window):
        - **Age ≤ 30 days from `completed_date:`** → Pass — `🔒 Completed prune manifest: [[_Archive/Snapshots/_prune-manifest (prune-YYYY-MM-DD-HHMMSS)]] is in the 30-day regret-recovery window ([N] days remaining). `/rollback` cascade-detection can use this to surface Tier B Log entries if the user needs to undo an approved closure.`
        - **Age > 30 days from `completed_date:`** → Nice to Have — `🧹 Expired completed prune manifest: [[_Archive/Snapshots/_prune-manifest (prune-YYYY-MM-DD-HHMMSS)]] is [N] days past the 30-day regret-recovery window. /clean removes these on its next run, or delete manually: rm "_Archive/Snapshots/_prune-manifest (prune-YYYY-MM-DD-HHMMSS).md".`
        - **Missing `completed_date:`** → Nice to Have — `🧹 Legacy completed manifest (no completed_date: to age against): [[_Archive/Snapshots/_prune-manifest (...)]]. Flip-verification may have silently missed adding the timestamp, OR this is from a pre-retention-policy prune. /clean treats as eligible for deletion.`
...
41. **Sync manifest aging + status check** — Detect stale OR in-progress `_sync-manifest (sync-YYYY-MM-DD-HHMMSS).md` files in `_Archive/Snapshots/`. Sidecar artifacts written by `/sync` Step 2.9 (skeleton) → Step 7.5 (flip to completed); consumed by `/rollback` Step 2.5b cascade detection.
    - **Vault-wide only** (skipped in scoped mode).
    ...
    - **T2.1 — status check (new)**: classify by `status:` FIRST:
      - **`status: in-progress`** → **Important** — `⚠️ In-progress sync manifest: [[_Archive/Snapshots/_sync-manifest (sync-YYYY-MM-DD-HHMMSS)]]. /sync Step 2.9 wrote the skeleton but Step 7.5 flip to "completed" did not land. Two possible causes: (1) /sync crashed or was interrupted mid-run — inspect the manifest body for Tier A snapshots and Tier B Log appends that landed; use /rollback to restore the snapshotted files if the partial sync is undesired. (2) /sync completed but Step 7.5 flip Edit silently missed — check whether the reported Tier B Log entries actually exist on the listed theses. If yes, the sync succeeded and this is a flip-only failure; manually edit the manifest frontmatter to status: completed and add today's date as completed_date. (3) /lint was run DURING an active /sync — if so, ignore this finding; the /sync run will flip status when it completes.`
      - **`status: completed`** → age-based classification below.
      - **Missing or malformed `status:`** → Important — `⚠️ Sync manifest [filename] has no parseable status: frontmatter. Legacy format (pre-T2.1) or corruption. Inspect manually — if it predates T2.1 introduction (2026-04-19 approx), it was written under the old "end-of-run-only" spec and has no in-progress state; treat as completed for aging purposes.`
    - **Age classification (applies to `status: completed` and legacy no-status manifests)**:
...
47. **Stress-test manifest aging + status check (T3.1)** — Detect `_stress-test-manifest (stress-test-YYYY-MM-DD-HHMMSS).md` files in `_Archive/Snapshots/`. Written by `/stress-test` Phase 4.6; consumed by `/rollback` Step 2.5d cascade detection for strikethrough review of append-only Log entries.
    - **Vault-wide only** (skipped in scoped mode).
    - **Precondition**: glob `_Archive/Snapshots/_stress-test-manifest*.md`. If none → Pass.
    - For each manifest, parse frontmatter (`type: stress-test-manifest`, `batch:`, `status:`, `ticker:`, `date:`).
    - **Severity by `status:`**:
      - `status: completed` → Pass (manifest is a recovery aid; age-based cleanup covers).
      - `status: in-progress` (rare — `/stress-test` writes the manifest as completed at Phase 4.6; an in-progress state would indicate the skill crashed between writing the research note and writing the manifest body) → Nice to Have — `🔖 In-progress stress-test manifest: [[_Archive/Snapshots/_stress-test-manifest (stress-test-YYYY-MM-DD-HHMMSS)]]. Unusual state — /stress-test normally writes manifest as completed in a single step. Inspect: if the corresponding research note and thesis Log entry exist, the stress test succeeded; manually edit to status: completed. If either is missing, stress-test partial failure; /rollback cascade will still work but consider re-running /stress-test TICKER.`
    - **Age classification** (applies to completed manifests):
      - `date` <= 90 days → Pass.
      - `date` 90–180 days → Nice to Have — `🔖 Stale stress-test manifest ([N] days old). Safe to delete once the corresponding research note is no longer actively referenced.`
      - `date` > 180 days → Important — `⚠️ Very stale stress-test manifest ([N] days old). The research note is typically still valuable as historical record; only the manifest's rollback-cascade utility has expired. Safe to delete.`
    - Do NOT delete manifests from `/lint`.

48. **Status manifest aging + status check (T2.2)** — Detect `_status-manifest (status-YYYY-MM-DD-HHMMSS).md` files in `_Archive/Snapshots/`. Written by `/status` Step 3.0.5 (skeleton) → Step 7.9 (flip to completed); consumed by `/rollback` Step 2.5e cascade detection for conviction/status transaction recovery.
    - **Vault-wide only** (skipped in scoped mode).
    - **Precondition**: glob `_Archive/Snapshots/_status-manifest*.md`. If none → Pass.
    - For each manifest, parse frontmatter (`type: status-manifest`, `batch:`, `status:`, `ticker:`, `transition_type:`, `date:`, optional `completed_date:`).
    - **Severity by `status:`**:
      - `status: in-progress` → Important — `⚠️ In-progress /status manifest: [[_Archive/Snapshots/_status-manifest (status-YYYY-MM-DD-HHMMSS)]]. /status Step 3.0.5 wrote the skeleton but Step 7.9 flip to completed did not land. Inspect the manifest body to determine which stages landed: (1) Thesis frontmatter change: check Theses/[ticker]; (2) Sector note edit: check Sectors/; (3) Archive move (closure only): check _Archive/[ticker]. If the transaction appears complete on inspection, manually edit manifest to status: completed. If incomplete, /rollback [batch] to restore pre-status state.`
      - `status: completed` → age-based classification.
      - Missing `status:` → Important per same pattern as #41.
    - **Age classification** (completed only): same thresholds as #41 (90/180 days).
    - Do NOT delete manifests from `/lint`.

49. **Thesis manifest aging + status check (H1)** — Detect `_thesis-manifest (thesis-TICKER-YYYY-MM-DD-HHMMSS).md` files in `_Archive/Snapshots/`. Written by `/thesis` Step 3.5 (skeleton) → Step 7.5 (flip to completed); consumed by `/rollback` Step 2.5f cascade detection for thesis-creation partial recovery.
    - **Vault-wide only** (skipped in scoped mode).
    - **Precondition**: glob `_Archive/Snapshots/_thesis-manifest*.md`. If none → Pass.
    - For each manifest, parse frontmatter (`type: thesis-manifest`, `batch:`, `status:`, `ticker:`, `proposed_name:`, `proposed_path:`, `sector:`, `date:`, optional `completed_date:`).
    - **Severity by `status:`**:
      - `status: in-progress` → Important — `⚠️ In-progress /thesis manifest: [[_Archive/Snapshots/_thesis-manifest (thesis-TICKER-YYYY-MM-DD-HHMMSS)]]. /thesis Step 3.5 wrote the skeleton but Step 7.5 flip to completed did not land. Inspect the manifest body to determine what landed: (1) Thesis file at [proposed_path]; (2) Sector note Active Theses entry in [sector]; (3) _hot.md sections (Active Research Thread, Recent Conviction Changes, Open Questions). If thesis file exists and all sections landed, manually edit manifest to status: completed. If partial (thesis exists but sector/hot.md incomplete), EITHER complete the remaining steps manually per manifest intended operations, OR rm the thesis file + manifest and re-run /thesis TICKER.`
      - `status: completed` → age-based classification.
      - Missing `status:` → Important per same pattern as #41.
    - **Age classification** (completed only): same thresholds as #41 (90/180 days).
    - **Pre-existing thesis check**: for each `in-progress` manifest, verify `Theses/[proposed_path]` exists. If the thesis file exists AND lint can read valid frontmatter, the manifest's `in-progress` state is likely a flip-failure (cause 2 per the message). If the thesis file does NOT exist, the skeleton was written but Step 4 failed — recovery is simpler (rm the manifest, re-run).
    - Do NOT delete manifests from `/lint`.
...
43. **Vault lock staleness detection** — Per `.claude/skills/_shared/preflight.md` Procedure 1.8, scan `.vault-lock*` files and classify by `timeout_at:`. The token-based lock model (C1/C2 fixes) does NOT rely on PID checks — Claude Code's stateless Bash tool makes per-Bash-block PIDs unreliable owners. Staleness is determined purely from `timeout_at:`.
    ...
    - For each lock file, read frontmatter (`token:`, `skill:`, `scope:`, `started_at:`, `timeout_at:`, optional `multi_scope:`).
    - **Classification**:
      - **Active** (`timeout_at:` > now): Pass silently. Lock is legitimately held.
      - **Stale** (`timeout_at:` < now): Important — `⚠️ Stale vault lock: [[.vault-lock...]]. Skill [X] (token [T]), scope [scope], started [started_at], timeout [timeout_at] ([N] minutes ago). The lock has exceeded its timeout budget. The prior run may have crashed, OR may still be legitimately running (long web-research, large batch operations). Recovery is USER-INITIATED — this skill does not auto-steal (see preflight.md §1.6). If you can confirm the prior run is abandoned: rm "[lock_path]". If the prior run's manifest exists (sync/status/prune/compare/stress-test manifest in _Archive/Snapshots/), /lint #36/41/47/48 surface in-progress state — consult those before removing.`
      - **Malformed** (missing required frontmatter fields): Important — `⚠️ Malformed vault lock: [[.vault-lock...]] missing required fields ([list]). Unknown ownership state. Safe to rm only if you can confirm no skill is actively running.`
    - **Multi-ticker scope** (lockfile has `multi_scope:` listing peer tickers): report all N peer locks together as a single multi-ticker entry. If some peer locks are missing, surface as `⚠️ Partial multi-ticker lock: token [T] expected locks on [peer list] but only [actual list] exist. Prior /compare run may have crashed mid-acquisition.`
