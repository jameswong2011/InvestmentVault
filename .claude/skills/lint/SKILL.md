...
35. **`_hot.md` schema integrity** — Verify canonical section layout (§5).
    ...
    - **Word-cap check**: total word count > 4,000 (soft cap per `_shared/hot-md-contract.md`) → Nice to Have: `🔖 _hot.md over soft cap ([N] words / 4,000). Skills auto-prune on next update via compression trigger order steps 1-5 (Sync Archive drop, Previous-line drop, OQ merge, pre-14d OQ cohort drop, pre-30d RCC roster-compress) — no action unless persistent across multiple runs.` Total word count > 5,000 (hard cap) → Important: `⚠️ _hot.md exceeds 5,000-word hard cap ([N] words). Next skill invocation runs full compression (steps 1-6 including aggressive RCC fallback). If still over hard cap after step 6, skill aborts _hot.md update — diagnose by checking whether the dashboard layer (Active Research Thread / Latest Sync) is itself oversized, since steps 1-6 cannot compress dashboard.`
