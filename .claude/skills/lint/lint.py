...
Usage:  python3 .claude/skills/lint/lint.py            # full vault
        python3 .claude/skills/lint/lint.py --ticker NVDA
...
def analytical(v, scoped_theses):
    ...
    for p, t in (scoped_theses or v["theses"]).items():
        ...
        # _shared/trigger-touch.md depends on. Section-ABSENT is #14's job (Conviction
        ...
        if weak:
            add("NICE", "#66", f"[[Theses/{p.stem}]] spine carries {len(weak)} weakly-sourced "
                ...
                f"the figure(s) (per _shared/provenance-tags.md)")
...
def writing_standards(v, scoped_theses):
    # #64 hedge-word ban (2026-07-15, CHG-15): CLAUDE.md Writing Standards forbids filler
        ...
        for i, ln in enumerate(t.split("\n"), 1):
            ...
            if m and hits < 3:
                add("NICE", "#64", f"{p.relative_to(VAULT)}:{i}: hedge word "
                    f'"{m.group().strip()}" — CLAUDE.md Writing Standards ban')
                hits += 1
