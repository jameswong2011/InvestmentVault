...
**The basic architecture** — Workflow 3 (the news pipeline) runs this sequence on each scheduled morning run:

1. **Read the config from the vault** — the LLM prompts, the tickers and themes to search, and the filtering / analysis-sensitivity settings — from the vault's `.md` files.
2. **Run parallel news searches** across five channels: Google News RSS, Brave, an open-source news-aggregation database, a financial-markets subscription feed, and a set of hand-selected outlet websites.
3. **Filter out duplicates** already seen in past runs (tracked in a JSON history database), scoring each survivor for how much genuinely new information it surfaces.
4. **Grade article quality**, dropping clickbait and AI slop.
5. **Cluster articles that report the same story** into one entry, and tag follow-up coverage of stories already briefed on earlier days.
6. **Summarise** — a final LLM writes the summary and analysis for each surfaced story.
7. **Output** a news report (organised by topic and ticker), a Telegram message with the top *N* stories, and a new entry in the covered-news database.
...
Machine currently runs Node v25.9.0 (non-LTS; n8n's engine check may refuse it). Install an LTS (Long-Term Support) release side-by-side via nvm (Node Version Manager):
...
Open `http://localhost:5678` → create the owner account (local credential, pick a real password). The timezone env vars make every cron expression below fire in AEST (Australian Eastern Standard Time).
...
- **No live workflow deposits into `_Inbox/`**: this contract is retained as the spec for any future re-sanctioned deposit.
...
| Benefit | **High.** 63 events/13 weeks in the current window. Dated observables (MRVL Q2 FY27 print late Aug, OCP Oct, re:Invent Nov–Dec) stop depending on memory |
...
⚠ **Second limitation (NOT accepted):** the deployed regex matches only single-wikilink ticker cells — `/catalyst` writes aliased multi-ticker cells (`[[path\|A]], [[path\|B]]`), which the parser skips entirely, so multi-thesis events never alert. Fixed parser in §4.1 card 4 — paste it over the deployed Code node at the next n8n touch.
...
**4 · Code** — ⚠ the step-3 parser in the card above is what's deployed; the block below is the **fixed parser** (apply at next n8n touch). Deployed-regex failure mode: `\[\[([^\]]+)\]\]\s*\|` requires the ticker cell to be exactly one wikilink — `/catalyst` writes aliased multi-ticker cells (`[[Theses/…\|CRCL]], [[Theses/…\|BTC]]`), which never match, so multi-thesis events are silently skipped. Fix: collapse wikilinks to their alias BEFORE cell-splitting — the alias `\|` inside `[[path\|TICKER]]` is exactly what breaks naive pipe-splitting:
...
**Governance:** Rule 1 — writes only new dated files into `Daily Intel/`; `_Inbox/` untouched. Rule 2 — LLM stages read bodies and write analytical summaries with coverage implications, brief-only; grounded-in-source with labeled inference. Vault ingestion, propagation, and conviction stay in `/ingest` + skills with full mental-models context.

---

**Step-by-step build** — §5.1–§5.7:

### 5.1 Pre-build verification (~15–20 min — do NOT skip)
...
```
PASS: clean markdown (headlines as text, no `<div>` soup). FAIL: `command not found` → `npm install -g defuddle`; `--markdown` unrecognized → `defuddle --help`, substitute the real flag in **card 17's Defuddle node**.
...
ls -t "/Users/alexcohen/InvestmentVault/Daily Intel/"*"X Intel.md" 2>/dev/null | head -1   # any hit → 𝕏 markers live; none → they stay silent until W5 runs
...
- **Re-arm after every edit.** n8n 2.x does NOT reliably re-register a scheduled workflow's cron trigger on Save — after editing/re-pasting any node in an *active* scheduled workflow (W1–W5), finish by toggling **Active OFF→ON**, or the next scheduled run silently won't fire (the workflow stays `active=1` but the cron is dropped). Manual "Execute Workflow" keeps working, which masks it.

**1 · Schedule Trigger** — one rule: **Trigger Interval** `Days` · **Days Between Triggers** `1` · **Trigger at Hour** `7am` · **Trigger at Minute** `10`. Daily morning run only. §5.6 cutover moves it to 07:00.
...
**2d · Execute Command "XDash"** — X attention overlay: `ls -t "/Users/alexcohen/InvestmentVault/Daily Intel/"*"X Intel.md" 2>/dev/null | head -1 | xargs cat 2>/dev/null` · On Error: Continue (no file → no 𝕏 markers). This glob must track the filename W5 writes (`dash_fname` in §7.3 Code D) — both were renamed from "X Dashboard" to "X Intel" together; if you rename the W5 output again, update this glob too or the 𝕏 markers silently vanish.
...
for (const sub of tun.split(/\n### /).slice(1)) {
  const fcl = (sub.split('\n')[0] || '').replace(/\s*\(.*$/, '').trim();   // ### heading, parenthetical stripped → brief group
  for (const r of sub.matchAll(/^\|\s*([^|]+?)\s*\|\s*(https?:\/\/[^|\s]+)\s*\|\s*([^|]+?)\s*\|\s*([^|]+?)\s*\|\s*([^|]+?)\s*\|\s*([^|]+?)\s*\|\s*([^|]+?)\s*\|\s*$/gm)) {
    ...
    tasks.push({ ch:'feed', id, url, cluster: fcl || cluster, bypass: triage==='no', body: !cfg.body_exempt.includes(id) });
  }
}
// Search targets — every query row PLUS one per-ticker query per thesis (quoted company name).
// Each row inherits its ### heading from _watchers.md as its cluster, so the brief groups under
// the registry's own categories (Custom silicon & compute, Foundry & packaging, …). 2026-07-27.
const targets = [];
for (const sub of section('News & Thematic').split(/\n### /).slice(1)) {
  const heading = sub.split('\n')[0].trim() || 'thematic';
  for (const r of sub.matchAll(/^\|\s*([^|]+?)\s*\|\s*([^|]+?)\s*\|\s*([^|]*?)\s*\|\s*([^|]+?)\s*\|\s*([^|]+?)\s*\|\s*$/gm)) {
    const [,idRaw,q,thesis,expires,status] = r.map(x=>String(x).trim());
    const id = idRaw.replace(/\s*⚠\s*$/,'');
    if (id==='id' || /^-+$/.test(id) || status!=='active') continue;
    if (expires!=='permanent' && expires < today) continue;
    targets.push({ id, q, cluster: heading });
  }
}
...
const gdeltRank = t => (t.cluster === 'ticker')
  ...
  : 1;                                                            // query watches (any ### cluster) → 1 (middle)
...
cfg.x_read = xread;
cfg.themes = targets.filter(t=>t.cluster!=='ticker').map(t=>`${t.id}: ${t.q}`).join(' · ');   // all query rows regardless of ### cluster (was ==='thematic' — would return empty after heading-inheritance and silently gut the {themes} token in triage/rescore/digest prompts)
...
**6b · Merge "FeedTasks"** — Mode: Append · **Number of Inputs `2`** · Input 1 ← Route output 0 (`feed`) · Input 2 ← Route output 1 (`gn`) · output → Feeds (card 7). Collapses the two RSS task streams into ONE delivery so the RSS node executes once. Without it, two wires into Feeds run the RSS node twice — and each delivery separately re-executes the entire Normalize→digest chain downstream. Both Route outputs always carry tasks, so neither input can starve this Merge.
...
**`limit=250` + `from`/`to` window**: FMP `news/stock` returns the 250 newest articles **across the whole batch**, recency-sorted — **NOT 250 per ticker**, and 250 is the endpoint's hard ceiling (verified: `limit=1000` still returns 250). The date window (= `max_age_d`) keeps those 250 in-scope instead of spent on stale items. Two coverage limits are structural, not tunable: (1) **US-listed symbols only** — foreign listings (Amsterdam/London/Tokyo/Toronto/Sydney) don't resolve in FMP's US news feed at any limit; they ride the GN/GDELT/Brave company-name channels instead. (2) **filename-prefix ≠ exchange-ticker** mismatches silently return 0 (e.g. KLA is KLAC on Nasdaq) — fixed by the `FMP_ALIAS` map in card 5's Plan; add a row when a US name shows `fmp 0` in the brief.
...
**12 · Code "Dedupe"** (Run Once for All Items) — cross-execution URL dedup in a **TTL-bounded static-data store** (replaces n8n's Remove Duplicates node, whose count-capped history grows without eviction and throws "exceeded cap" — Google News redirect URLs churn every fetch). This version is bounded by *time*, not count: it forgets any URL older than `max_age_d + 2` days every run, so it physically cannot overflow and needs no manual clearing.
```javascript
...
return out;
```
Notes: `$getWorkflowStaticData` is sandbox-available (unlike `new URL`); the store persists across *scheduled* runs and stays bounded at `dedup_ttl_d` days of URLs (self-pruning). This node only stops **exact-URL re-processing** from the stable channels (feed/GDELT/Brave/FMP) — it does NOT deduplicate *stories*. Cross-run and semantic same-story dedup belong to the embeddings SemCluster (15e), not here. Hard-reset: `$getWorkflowStaticData('global').seenUrls = {}`.
...
- **Output → card 18c "SumPrep"** — the single merged stream (every representative, body and no-body together, plus the sentinel carrying `_stats`/`_followups`). Wires **straight to SumPrep**.
...
if (picks.length) {
  ...
  let tg = 'News Brief: quiet run — 0 stories admitted';
  if (warn.length) tg += `\n⚠ ${warn.length} source failures`;
  out.push({ json: { type:'tg', text: tg.slice(0,3900) } });
}
return out;
...
Each of the three outputs feeds its sink in **card 21**. No clip lane, no `_Inbox/` writes.
...
- **Read/Write Files from Disk "WriteDigest"** — Operation **Write File to Disk** · Input Binary Field `data` · File Path → Expression (by-name — Convert to File **strips the item's JSON on this instance**, so `$json.fname` arrives empty and the path collapses to the bare directory → "path is a directory" error; the by-name form reaches past the Convert node to Assemble, where the JSON still exists):
```
...
### 5.9 Email delivery (OPTIONAL)

Optional add-on — email the **News Brief** (W3) and/or **X Intel** (W5) to a registry-driven BCC mailing list on every run. Skip this section entirely if you don't want email; nothing else depends on it. Two new nodes per workflow (`MailPrep` Code node + `Send Email` node); no existing node changes.

**Registry — add a `## Mailing list` section to `_watchers.md`:** one email per bullet (e.g. `- alice@example.com` — replace with real addresses; `example.com` won't deliver). The workflow parses them at runtime and BCCs everyone (recipients never see each other). Empty list = mail goes only to the `To` address (you).

**SMTP credential (one, shared) — pick one:**
- **Gmail:** Google Account → Security → 2-Step Verification ON → App passwords → generate. n8n → Credentials → SMTP: Host `smtp.gmail.com` · Port `465` · SSL ON · User = Gmail · Password = the app password.
- **Proton (send from a Proton address):** needs a PAID Proton plan + **Proton Mail Bridge** running on the Mac. Install Bridge, sign in. Bridge → account → Mailbox details gives: Host `127.0.0.1` · Port `1025` · STARTTLS · User = Proton address · Password = the Bridge password. Export Bridge's TLS cert, then make Node trust it (macOS Keychain does NOT apply to Node): `export NODE_EXTRA_CA_CERTS="/path/to/cert.pem"; pm2 restart n8n --update-env; pm2 save`. n8n → Credentials → SMTP: Host `127.0.0.1` · Port `1025` · SSL OFF · User = Proton address · Password = Bridge password · Client Host Name blank.

**W3 — `MailPrep` (Code, Run Once for All Items)** — wire from `Switch "Out"` output 0 (`digest`):

```javascript
const md = $('RegText').first().json.data || '';
const sec = (md.split('## Mailing list')[1] || '').split('\n## ')[0];
const bcc = [...new Set((sec.match(/[\w.+-]+@[\w-]+\.[\w.-]+/g)) || [])].join(',');
return [{ json: {
  bcc,
  subject: 'News Brief — ' + ($json.fname || '').slice(0, 10),
  body: $json.content || ''
} }];
```

**W5 — `MailPrep` (Code, Run Once for All Items)** — wire from `Assemble`:

```javascript
const md = $('Extract Watchers').first().json.data || '';
const sec = (md.split('## Mailing list')[1] || '').split('\n## ')[0];
const bcc = [...new Set((sec.match(/[\w.+-]+@[\w-]+\.[\w.-]+/g)) || [])].join(',');
const a = $('Assemble').first().json;
return [{ json: {
  bcc,
  subject: 'X Intel — ' + (a.dash_fname || '').slice(0, 10),
  body: a.dash_body || ''
} }];
```

**`Send Email` (both workflows)** — wire from `MailPrep`: Credential = your SMTP · From Email / To Email = your address · Subject → Expression `{{ $json.subject }}` · Email Format `Text` · Text → Expression `{{ $json.body }}` · Options → Add Option → BCC Email → Expression `{{ $json.bcc }}` · Settings → On Error `Continue (using regular output)`. On-Error-Continue is mandatory — a mail failure (or Bridge not running) must never abort the run before the file writes.
...
**Intent guard:** the digest is a scanning surface — you pick the 1–3 links worth a manual `/ingest`. Auto-ingesting news bodies wholesale is deliberately not built.

---

## 6. Workflow 4 — X Canary (+ shared X setup)

Daily provider-health probe for the X stack — 4 nodes, built first because it end-to-end tests credential → search → Telegram before the big workflow investment, then stays on as the canary (silent thin results → loud alert). §6.0–§6.3 are one-time setup shared with Workflow 5. Both **live**.
...
**1 · Schedule Trigger** — **Trigger Interval** `Days` · **Days Between Triggers** `1` · **Trigger at Hour** `8am` · **Trigger at Minute** `30` (daily is the live cadence).
...
**21 · Telegram** (from `XFanout`) — existing credential · **Chat ID** `1779654963` · **Text** → Expression → `{{ $json.text }}` · **Additional Fields → Add Field → 'Append n8n Attribution' → OFF** · **Settings → On Error** `Continue (using regular output)`. Sends one message per input item (the fan-out) — Telegram's ~1 msg/s flood limit means ≤10 is safe; if 429s appear, lower `x_tg_max_msgs`.
- n8n's default-on attribution footer forces an implicit Markdown parse of the whole message — leaving Parse Mode unset is NOT enough because the footer still triggers parsing. Turning attribution off sends plain text; alternatively strip `* _ [ \` ` from the message text in a Code node.
- **On Error `Continue` is mandatory:** the dashboard write (18→19) and state write (16→17) are separate branches off `Assemble`. A Telegram failure with the default `Stop Workflow` aborts the entire execution — including those writes — so a formatting hiccup in one tweet would silently lose the run's engagement-ledger update. Continue-on-error keeps the writes safe.
...
| Divergence feels generic — thesis never referenced | Verify the payload, not the wiring: open the execution → `Sentiment` → input → search `MY THESIS` — it must be followed by full section text, not just a heading. |
...
- **Monthly review (~20 min):** prune expired/orphaned rows in [[_watchers.md]] (move to its Retired section), retune noisy queries, tripwire levels vs current Conviction Triggers, triage threshold, pm2 status. This single file is the whole "what am I tracking" surface — one read tells you everything n8n is pulling. (The W3 Dedupe store self-prunes via TTL — no manual clearing needed.)
...
- **2026-07-24 (double-run ghost cron + W5 Telegram parse-entities, user reports)** — (1) W3 fired twice one morning: a duplicate active "UC3 - Newsweep" (`BawLt64…`) ran via a **ghost cron** — its schedule trigger node had been deleted, but n8n 2.x keeps the in-memory cron (registers at activation, never re-scans on node deletion), so removing the trigger couldn't stop it. Fix: `pm2 restart n8n` rebuilds the schedule registry from the actual node definitions → no trigger node, no cron; superseded duplicate workflows deleted. (2) W5 Telegram 400 `can't parse entities` + no disk writes: the Telegram node's parse mode tried to Markdown-parse raw tweet text (`*`/`_`/`[`), and at the default `Stop Workflow` the 400 aborted the run before the dashboard/state writes. Fix (cards 21 W5 + 21c W3): **Parse Mode `None`** + **On Error: Continue**. Doc gap closed — the Telegram cards never specified parse mode.
...
| A workflow fires on schedule despite having NO trigger node / two copies run on one cron | **Ghost cron** — n8n 2.x registers crons in memory at activation and does NOT re-scan when you delete a trigger node, so the cron keeps firing (the reverse of the dropped-trigger row above). Deleting the trigger — or even deactivating in the UI — may not clear it | **`pm2 restart n8n`** rebuilds the schedule registry from the actual node definitions, so a workflow with no trigger registers no cron. Then delete superseded duplicate workflows — a deleted workflow can never fire (export to JSON first if you want a backup) |
