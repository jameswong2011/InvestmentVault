...
| `cluster_model` | `## Outlet Feeds → ### Tuning` | `claude-opus-4-8` | W3 story clustering + repeat detection (§5.3 card 18b) — Opus for semantic same-story judgement |
...
const DEF = { triage_min:7, triage_min_pw:9, gdelt_spacing_s:8, brave_budget_mo:3500, story_memory_days:7, catalyst_window_d:10, max_age_d:3, tg_max_msgs:10, tg_per_subject:2, dedup_ttl_d:3, track_min_score:8, track_window_d:30, merge_jaccard:0.42,
  triage_model:'claude-sonnet-5', cluster_model:'claude-opus-4-8',
...
const cfg = {...DEF};
for (const k of ['triage_min','triage_min_pw','gdelt_spacing_s','brave_budget_mo','story_memory_days','catalyst_window_d','max_age_d','tg_max_msgs','tg_per_subject','dedup_ttl_d','track_min_score','track_window_d','merge_jaccard']) {
...
return out;
```
Notes: `$getWorkflowStaticData` is sandbox-available (unlike `new URL`); the store persists across *scheduled* runs and stays bounded at `dedup_ttl_d` days of URLs (self-pruning). This node only stops **exact-URL re-processing** from the stable channels (feed/GDELT/Brave/FMP) — it does NOT deduplicate *stories*. Cross-run and semantic same-story dedup belong to the cluster/story-memory LLM (18a/18b), not here (see §11 2026-07-21 note on why deterministic string-matching can't own this). Hard-reset: `$getWorkflowStaticData('global').seenUrls = {}`.
...
const payload = JSON.stringify(items.map((j,k)=>({i:k, t:j.title, src:j.source||j.feedId,
  x:(j.bodyOk ? j.bodyText : (j.snippet||'')).slice(0,600).replace(/\s+/g,' ').trim() })));   // excerpt → LLM judges same-story on CONTENT, not just headline
...
return [{ json: { _items: items, _preFollowups: preFups, _warnings: sentinel._warnings || [], _prior: priorList,
  ...
  _llm_body: !items.length ? null : {
    model: cfg.cluster_model, max_tokens: 24000, thinking: { type: 'adaptive' },   // headroom for thinking + JSON — 8000 let thinking truncate the clusters JSON → SumPrep fell to singletons (no clustering)
...
`cluster_model` defaults to `claude-opus-4-8` (Opus for the semantic same-story judgement — the payload now carries per-item excerpts); ~6–8k tokens/run + story memory ≈ $8–12/mo. The same call performs **cross-run story memory**: PRIOR STORIES from the last `story_memory_days` of run logs ride in the prompt (ClusterPrep packed them into `_llm_body`), and follow-up coverage that adds no material new facts is diverted to a links-only ♻ section instead of being re-summarised; items that *advance* a prior story are treated as new.
...
for (let a=0;a<items.length;a++) { if (repeatIdx.has(a)) continue;
  for (let b=a+1;b<items.length;b++) { if (repeatIdx.has(b) || find(a)===find(b)) continue;
    ...
    if (inter/(A.size+B.size-inter) >= (cfg.merge_jaccard || 0.42)) par[find(b)]=find(a); } }   // registry-tunable: lower = more merging
const comp = {};
...
- **(k) Story memory** (day 2+) — `_prior` non-empty in the 18a execution; repeats parked in ♻.
...
| `Method Not Allowed` / 405 on an LLM node | The HTTP node's **Method** is still n8n's GET default — Anthropic is POST-only | Set **Method: POST**. Applies to all four LLM nodes (Triage/Rescore/Cluster/Summarise) |
...
| Same story re-briefed across runs/days | Story memory not reaching the cluster call (`_prior` empty in 18a → PriorStories broken), or the LLM repeat bar too loose | Check `_prior` AND `_preFollowups` in ClusterPrep's output; the title-key guard diverts near-identical titles pre-LLM; reworded repeats rely on the bias-toward-repeat clause — sharpen it or widen `story_memory_days` |
...
| Real development buried in ♻ Follow-up coverage | Repeat bar too loose | Tighten the "ADVANCES the story" clause in 18a; the ♻ link is kept either way — nothing lost, only unsummarised |
...
| Same story appears as two entries in ONE brief | Cluster under-grouping | Three guards: publisher-tail stripping in title keys, SumPrep's Jaccard force-merge (≥0.5), the LLM grouping rule; last lever `cluster_model` → Opus. Adjust the 0.5 threshold in SumPrep |
