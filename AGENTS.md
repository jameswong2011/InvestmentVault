<!-- Generated from CLAUDE.md by .agents/port_claude_skills.py. Edit the canonical source, then regenerate. -->
...
11. **Skill-content loading safeguard** — before any write phase, read the full `.agents/skills/<skill>/SKILL.md` whenever the workflow arrived as metadata only, a partial excerpt, or text containing a truncation marker. Never reconstruct missing steps from memory. Long skills (`$sync`, `$status`, `$deepen`, `$compare`, `$thesis`, `$rollback`) contain late-stage safety gates that must be loaded from disk before mutation.
...
| **Pinned** | header contains `[[pinned]]` (fresh OR addressed state) | Persistent revisit slot — Codex addresses normally when asked; the marker exempts the callout from `$archive-callouts` 180-day sweep so it stays in its original section indefinitely; user can re-open later (delete `→ Addressed`/`Prompt`/`Response` blocks; marker survives) for re-address with newer data |
...
- The user's original prompt body goes inside `*...*` italic markers — distinguishing user input vs. Codex's analytical reply.
...
- `$lint`: enforces archive schema (checks #50–#53 + #56) — sweep freshness (#50), stale-fresh callouts (#51), malformed Legacy entries (#52), orphan Legacy sections (#53), deprecated `[[preserve]]` markers → migrate to `[[pinned]]` (#56). (#44–#48 are scenario-reversal + the compare/registry/stress/status manifest checks, not callout hygiene.)
