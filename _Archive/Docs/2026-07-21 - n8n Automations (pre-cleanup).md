...
| `cluster_model` | `## Outlet Feeds → ### Tuning` | `claude-sonnet-5` | W3 story clustering (§5.3 card 18b) |
...
| `cluster_prompt` | `## Outlet Feeds → ### Prompts` | card-5 `DEF_P` fallback | W3 clustering + repeat detection (tokens: `{items}` `{prior}`) |
...
```javascript
const DEF = { triage_min:7, triage_min_pw:9, gdelt_spacing_s:8, brave_budget_mo:3500, story_memory_days:7, catalyst_window_d:10, max_age_d:3, tg_max_msgs:10, dedup_ttl_d:3, track_min_score:8, track_window_d:30,
  triage_model:'claude-sonnet-5', cluster_model:'claude-sonnet-5',
...
const cfg = {...DEF};
for (const k of ['triage_min','triage_min_pw','gdelt_spacing_s','brave_budget_mo','story_memory_days','catalyst_window_d','max_age_d','tg_max_msgs','dedup_ttl_d','track_min_score','track_window_d']) {
  const m = tun.match(new RegExp('\\|\\s*'+k+'\\s*\\|\\s*([^|]+?)\\s*\\|'));
  if (m && !isNaN(parseFloat(m[1]))) cfg[k] = parseFloat(m[1]);
}
for (const k of ['triage_model','cluster_model','rescore_model','digest_model']) {
  const m = tun.match(new RegExp('\\|\\s*'+k+'\\s*\\|\\s*([^|]+?)\\s*\\|'));
  if (m) cfg[k] = m[1].trim();
}
...
```
https://api.gdeltproject.org/api/v2/doc/doc?query={{ encodeURIComponent($json.q) }}&mode=artlist&format=json&maxrecords=50&timespan=24h
```
...
```
https://financialmodelingprep.com/stable/news/stock?symbols={{ $json.symbols }}&limit=50
```
Its **main output wires into PreNormalize (card 10b) input 5** — the last of the five fetch wires.
...
for (const it of $input.all()) {
  ...
  if (seen[k]) continue;                                // seen within TTL → drop as duplicate
  seen[k] = now; out.push(it);
}
return out;
```
Notes: `$getWorkflowStaticData` is sandbox-available (unlike `new URL`); the store persists across *scheduled* runs and stays bounded at ~`max_age_d`-days of URLs (a few thousand entries, self-pruning). Cross-run GN *story* repeats are handled by story memory (18a), not here — this node only stops exact-URL re-processing from the stable channels (feed/GDELT/Brave/FMP). To hard-reset if ever needed, run a one-line Code node `$getWorkflowStaticData('global').seenUrls = {}` once; but TTL eviction means you never have to.
...
> **Design note — nothing to build in this box.** The four LLM calls (Triage 14a · Rescore 17 · Cluster 18b · Summarise 18d) share one contract. (1) The request body is **built as a plain object in the preceding Code node** (cards 13, 17.4, 18a, 18c each set `_llm_body`); the HTTP node only stringifies it via the card-14a Expression paste, identical on all four — never assemble JSON by string-templating, because a quote character in any headline would produce invalid JSON. (2) **No `temperature` on any call**: Opus-family models reject sampling parameters with a 400, which On-Error-Continue would swallow into a silent headline-only digest; omitting it everywhere keeps every §2.4 model cell freely swappable. **Adaptive thinking rides on Rescore, Cluster, and Summarise** (`thinking: { type: 'adaptive' }` — a reasoning control, not a sampling param; valid on Opus 4.8 and Sonnet 5, and the parsers already filter to `text` blocks so thinking blocks pass through harmlessly); Triage stays plain — it's the volume stage. (3) **Models come from the registry** (§2.4) via Plan's `cfg` — currently triage + cluster `claude-sonnet-5`, rescore + digest `claude-opus-4-8`; de-escalation levers in §2.4 and the §5.5f audit. (4) **Prompts come from the registry too** — `## Outlet Feeds → ### Prompts` `####` blocks, parsed by Plan into `cfg` with runtime token substitution (`{tickers}` `{themes}` `{items}` `{prior}`); a block missing its required token reverts to the card-5 `DEF_P` fallback. Edit prompts in Obsidian, never in the Code nodes.
...
**14a · HTTP "Triage"** — the template LLM node; build it once, then copy-paste for 17.5/18b/18d:
...
```
Prompt and model live in card 13's `_llm_body` — this field never changes across the four LLM nodes. All four are **POST** with this identical body Expression; the fastest build is to finish this node, then copy-paste it for Rescore (17.5) / Cluster (18b) / Summarise (18d) and only swap nothing inside — the differing payload comes from each one's preceding Code node. **Wire Triage's output → card 15 (Admit).**
...
**16 · IF "Body?"** (from PreBody) — one condition · Value 1 → Expression (Boolean · **is true**):
```
{{ $json.fetchBody === true }}
```
true → card 17 · false → card 18 (Merge input 1; carries no-body admitted items, headline-only bypass rows, and the warnings sentinel).
...
- **Output → card 18a "ClusterPrep"** — the single merged stream (every admitted item, body and no-body together).

Append mode (not a keyed merge) because the two lanes hold *different* items, not two halves of the same item — Rejoin concatenates them. Like PreBody (15b), an explicit Merge here — not two wires into 18a's single port — keeps ClusterPrep executing once, not once per lane.

**18a · Code "ClusterPrep"** (Run Once for All Items, from Rejoin) — ⬥ NEW: collects admitted items and builds the clustering payload (headlines + sources only — cheap):
...
const cand = all.filter(j=>!j._warnings && (j.admitted || j.bypass));
// story memory: parse recent run logs (card 2b) so follow-up coverage of briefed stories is recognised
...
((($('PriorStories').first() || {}).json || {}).stdout || '').split('@@@').forEach(seg => {
  try { const run = JSON.parse(seg.trim());
    if (run.date >= cutoff) for (const s of (run.stories||[]))
      prior.push({ t: s.title, sum: (s.sum||'').slice(0,150) });
  } catch(e) {}
});
const priorList = prior.slice(0,250).map((s,k)=>({p:'P'+k, t:s.t, sum:s.sum}));
// deterministic cross-run guard — a title-key match against prior stories auto-diverts to ♻ BEFORE the LLM votes
const tkey = t => String(t||'').replace(/\s+[-–—|]\s+[^-–—|]{2,40}$/,'').toLowerCase().replace(/[^a-z0-9 ]/g,'').split(/\s+/).filter(w=>w.length>3).sort().slice(0,8).join(' ');
const priorByKey = new Map(prior.map(p=>[tkey(p.t), p.t]).filter(([k])=>k));
const preFups = [], items = [];
for (const j of cand) { const k = tkey(j.title);
  if (k && priorByKey.has(k)) preFups.push({ title:j.title, curl:j.curl, source:j.source||j.feedId, prior: priorByKey.get(k) });
  else items.push(j); }
const payload = JSON.stringify(items.map((j,k)=>({i:k, t:j.title, src:j.source||j.feedId})));
const priorPayload = JSON.stringify(priorList);
return [{ json: { _items: items, _preFollowups: preFups, _warnings: sentinel._warnings || [], _prior: priorList,
  _stats: { seen: sentinel._seen ?? null, fresh: sentinel._new ?? null, admitted: cand.length, ch: sentinel._ch || null, stale: sentinel._stale ?? 0 },
  _llm_body: !items.length ? null : {
    model: cfg.cluster_model, max_tokens: 8000, thinking: { type: 'adaptive' },
    messages: [{ role: 'user', content: sub(cfg.cluster_prompt,
      {'{items}':payload, '{prior}':priorPayload}) }] } } }];
```

**18b · HTTP "Cluster"** — **from ClusterPrep (18a)** · **→ SumPrep (18c)**. ⬥ NEW: story consolidation — the 5-channel design means the same event arrives with different URLs and reworded headlines from several outlets; URL dedupe can't catch that. Same node shape as card 14a — build it identically:

- Method **POST** · URL `https://api.anthropic.com/v1/messages`
- **Credential:** Anthropic Header Auth (supplies the `x-api-key` header automatically — do **not** add it by hand)
- **Header** — Send Headers ON → Add Parameter → **Name** `anthropic-version` · **Value** `2023-06-01` (both plain fixed strings, no Expression)
- **On Error: Continue** (a failed or null-body cluster call degrades to one-story-per-item with zero repeat-detection — never kills the run)
- **Body** — Send Body ON → Body Content Type `JSON` → Specify Body `Using JSON` → toggle the JSON field to **Expression** → paste:
```
{{ JSON.stringify($json._llm_body) }}
```

`cluster_model` defaults to `claude-sonnet-5`; ~3k tokens/run + the story memory ≈ $2–3/mo. The same call performs **cross-run story memory**: PRIOR STORIES from the last `story_memory_days` of run logs ride in the prompt (ClusterPrep packed them into `_llm_body`), and follow-up coverage that adds no material new facts is diverted to a links-only ♻ section instead of being re-summarised; items that *advance* a prior story are treated as new.

**18c · Code "SumPrep"** (from Cluster) — ⬥ builds one summary payload per STORY (12 stories/batch); representative = best-scored body-bearing member; excerpts merge the top 2 members' texts; unclustered or invalid indices fall back to singleton stories:

```javascript
const cfg = $('Plan').first().json.cfg;
const sub = (t,m)=>Object.entries(m).reduce((s,[k,v])=>s.split(k).join(v ?? ''), t);
const src = $('ClusterPrep').first().json;
const items = src._items || [];
let clusters = null, repeats = [];
try { const txt = ($json.content||[]).filter(b=>b.type==='text').map(b=>b.text).join('');
  const om = txt.match(/\{[\s\S]*\}/);
  if (om) { const o = JSON.parse(om[0]); clusters = o.clusters; repeats = o.repeats || []; }
  else { clusters = JSON.parse(txt.match(/\[[\s\S]*\]/)[0]); }   // bare-array fallback: clusters only
} catch(e) {}
// repeats → follow-up links, excluded from story building (never silently dropped — rendered in the brief's tail)
const priorMap = Object.fromEntries((src._prior||[]).map(p=>[p.p, p.t]));
const repeatIdx = new Set(), followups = (src._preFollowups || []).slice();   // deterministic 18a diversions seed the ♻ list
for (const r of (Array.isArray(repeats)?repeats:[])) {
  const [n, pref] = Array.isArray(r) ? r : [null, null];
  if (Number.isInteger(n) && n>=0 && n<items.length && !repeatIdx.has(n)) {
    repeatIdx.add(n);
    const it = items[n];
    followups.push({ title: it.title, curl: it.curl, source: it.source||it.feedId, prior: priorMap[pref] || null });
  }
}
// grouping = LLM clusters ∪ deterministic near-dup merge — token-overlap union-find backstops the model,
// so mechanically-similar headlines CANNOT render as two stories regardless of the model's verdict
const normT = t => String(t||'').replace(/\s+[-–—|]\s+[^-–—|]{2,40}$/,'').toLowerCase().replace(/[^a-z0-9 ]/g,' ').split(/\s+/).filter(w=>w.length>3);
const sets = items.map(j=>new Set(normT(j.title)));
const par = items.map((_,i)=>i); const find = x => par[x]===x ? x : (par[x]=find(par[x]));
if (Array.isArray(clusters)) for (const c of clusters) {
  const idx = (Array.isArray(c)?c:[c]).filter(n=>Number.isInteger(n)&&n>=0&&n<items.length&&!repeatIdx.has(n));
  for (let i=1;i<idx.length;i++) par[find(idx[i])]=find(idx[0]);
}
for (let a=0;a<items.length;a++) { if (repeatIdx.has(a)) continue;
  for (let b=a+1;b<items.length;b++) { if (repeatIdx.has(b) || find(a)===find(b)) continue;
    const A=sets[a], B=sets[b]; if (A.size<3 || B.size<3) continue;
    let inter=0; for (const w of A) if (B.has(w)) inter++;
    if (inter/(A.size+B.size-inter) >= 0.5) par[find(b)]=find(a); } }
const comp = {};
items.forEach((_,n)=>{ if (!repeatIdx.has(n)) (comp[find(n)]=comp[find(n)]||[]).push(n); });
const groups = Object.values(comp);   // every non-repeat lands in exactly one component — LLM gaps become singletons automatically
const fin = j => (j.s2 !== undefined && j.s2 !== null) ? j.s2 : j.s;
const stories = groups.map(idx => {
  const mem = idx.map(n=>items[n]);
  const best = [...mem].sort((a,b)=>((b.bodyOk?1:0)-(a.bodyOk?1:0)) || ((fin(b)??0)-(fin(a)??0)));
  const scores = mem.map(m=>fin(m)).filter(s=>s!==null&&s!==undefined);
  const sTick = [...new Set(mem.map(m=>(m.feedId||'').startsWith('tk-')?m.feedId.slice(3).toUpperCase():null).filter(Boolean))];
  ...
  return { members: mem.map(m=>({title:m.title, curl:m.curl, source:m.source, feedId:m.feedId, ch:m.ch})),
    title: best[0].title, cluster: best[0].cluster, sig,
    score: scores.length ? Math.max(...scores) : null,   // null (not 0) when no member was ever scored
    bodyOk: mem.some(m=>m.bodyOk),
    x: best.slice(0,2).map(m=>(m.bodyOk?m.bodyText:(m.snippet||'')).slice(0,4500)).join('\n---\n') };
});
const out = [];
for (let i=0;i<stories.length;i+=8) {                       // 8 stories/call — more per-story attention than 12
  ...
  out.push({ json: { _batch: chunk.map(({x, ...rest})=>rest),   // strip excerpts — members are already light
    _warnings: i===0 ? (src._warnings||[]) : [], _stats: i===0 ? src._stats : null,
...
return out.length ? out : [{ json: { _batch: [], _warnings: src._warnings||[], _stats: src._stats, _followups: followups, _llm_body: null } }];
```
...
// Telegram fan-out — top-N stories, one message each (tg_max_msgs registry cap); the Telegram node sends one message per tg item
const ranked = [...stories].sort((a,b)=>(b.score??-1)-(a.score??-1));
const nMsg = Math.max(1, Math.min(20, Math.round(cfg.tg_max_msgs ?? 10)));
const picks = ranked.slice(0, nMsg);
if (picks.length) {
  picks.forEach((t, i) => {
    let tg = `★ ${t.title}`;
    if (t.sum) tg += `\n${t.sum}`;
...
`1→2→2b→2c→2d→3→4→5→6` · **6:{0,1}→6b(Merge FeedTasks)→7** · 6:2→8(loop: Gdelt→GParse→Wait→loop) · 6:3→9(loop: Brave→BParse→Wait→loop) · 6:4→10 · **{7 main→in1, 7 error→in2, 8 done→in3, 9 done→in4, 10→in5}→10b(Merge PreNormalize)→11**→12→13→14 · 14-true→14a→15→15b(input 1) · **14-false→15b(input 2)** · 15b→16 · 16-true→17(BodyLoop: Defuddle→Body1→loop; done→RescorePrep→Rescore→Final) · {16-false, 17-Final}→18→18a→18b→18c→18d→19→20(Switch Out)→**21 sinks**{0:ConvertDigest→WriteDigest · 1:ConvertLog→WriteLog · 2:Notify}. The 6b/10b Merges are mandatory, not optional: per-branch execution of multi-wire nodes was observed on this instance at first build (2026-07-20) — direct wiring runs Normalize (and the whole chain after it) once per delivery.

### 5.5 First run

**LLM-stage timing profile** (what "normal" looks like per node — durations scale with content volume, so incremental re-runs are legitimately fast): Triage = ceil(scored/120) calls, seconds-to-minutes · Body loop = one defuddle per article · Rescore = ceil(bodyItems/10) calls at ~3–5s each (a 5s Rescore = one thin batch, normal on incremental runs) · **Cluster = always exactly ONE call**, 10–30s (scales with admitted count + `story_memory_days` prior entries, never with call count; <1s = failed/null call → one-story-per-item degradation) · Summarise = ceil(stories/12) calls, the slowest stage post-analytical-prompt. Fast stages on a small run are health, not failure — the funnel counts, not the clock, are the diagnostic. With adaptive thinking on Rescore/Cluster/Summarise, expect those stages to run 2–4× longer than the plain-call baselines above — reasoning tokens, not a hang.

Manual **Execute Workflow** (a manual run never delays the schedule). Expected: run completes in **~25–35 min** (counted at build: 119 GDELT targets — 82 tickers + 37 thematic rows — × 8s pacing ≈ 18–20 min for that leg alone), and the channels run **sequentially, not in parallel** — n8n executes one branch at a time, so RSS finishes first (a minute-plus of serial feed fetches) before GDELT/Brave/FMP even start; PreNormalize (10b) holds until all five inputs deliver, so arrival order is irrelevant. Wall-clock is the SUM of the channel legs (GDELT Wait-pacing across ~100 targets dominates; body loop second) · one brief in `Daily Intel/` with per-story Opus summaries + all-source links, grouped by cluster · Telegram top-5. Then check: (a) admitted count sane (tens, not hundreds — if hundreds, raise `triage_min`), (b) **spot-check 3 summaries against their articles** — every stated fact must appear in the source text (hallucination check; `*(headline only)*` items get least trust), (c) summaries are grounded — every stated fact must trace to the source text and inference must be labeled ("implies"/"suggests"); a number or claim absent from the article is the failure to catch (the analytical layer may opine, never invent), (d) `⚠ Source failures` list — dead feeds to prune from the registry, (e) ticker-cluster items resolve to the right companies (a wrong-company match means the thesis filename's company name is ambiguous — add a distinguishing word to the filename or a thematic row), (f) **triage-band audit** (calibration period): open the execution → `Admit` node input → scan items scored 5–6 (killed by `triage_min` 7) for anything material to a live thesis. Structural protections limit any single miss (5-channel redundancy re-surfaces stories under different headlines; `triage: no` sources bypass scoring; `triage_min` is a free recall lever). Two levers from the audit: real misses in the band → lower `triage_min` (headlines rarely reward a model above Sonnet); band is clean noise → `triage_model` back to `claude-haiku-4-5` is a ~$15–25/mo saving with nothing lost, (g) **loop aggregation**: open GdeltLoop's *done* output — it must contain article-shaped items (`title`/`url`), not task items; if it emits tasks, insert an Aggregate node collecting GParse/BParse outputs ahead of card 11 (same check for BodyLoop → RescorePrep), (h) **write lanes**: confirm the dated brief landed in `Daily Intel/` AND the dated JSON in `.data/news_stories/` — an empty filename means Convert to File dropped the item JSON; apply the card-20 by-name fallback, (i) **funnel header populated**: `? fetched → ? new` in the brief means the warnings sentinel was lost upstream — check it survives Dedupe (unique `curl`) and the 14-false→15b→16-false path; the `Channels:` line below it must show all five channels non-zero on a weekday (a zero names the broken channel — this line is the standing early-warning as sources get added), (j) **exactly one digest + one Telegram push + one story log** — duplicates mean a multi-wire node executed per branch; the 6b/10b Merges are the guard (per-branch execution is CONFIRMED on this instance — verify both Merges are wired per §5.4 before anything else), (k) **story memory** (from day 2): yesterday's stories reappearing as full entries means PriorStories isn't feeding the cluster call — check the `.data/news_stories/` logs exist and card 18a's `_prior` list is non-empty in the execution; a genuinely-new development wrongly parked in ♻ means the repeat bar is too loose — tighten the "ADVANCES the story" line in the 18a prompt or shorten `story_memory_days`, (l) **context markers**: a ticker story near a known catalyst shows `📅 T-N: event` (absent for all stories = `_catalyst.md` stale or Catalyst node errored — both benign, run `/catalyst` to refresh) and X-tracked names show `𝕏 <sentiment>` from the latest W5 dashboard (`𝕏 quiet` on a high scorer = the crowd hasn't found it — the pre-consensus case worth a look). Publish (Active toggle) only after one clean manual run. First scheduled runs re-surface some items legacy W3 already showed — fresh dedupe store, self-heals in a day. Expect the first run's brief to be large (GN backlog): a one-time ~$5–10 triage+summary spend; if that bothers, temporarily raise `triage_min` to 8 for run 1 and revert.
...
| Summaries editorialize or infer thesis impact | Sonnet drifting past the factual-brief prompt | Tighten the 18d prompt (the "no adjectives of significance" line is the lever); spot-check against source |
...
| Every story `*(headline only)*`, no summaries | An LLM node is 400ing silently (On-Error-Continue) — usually a sampling param on an Opus model or a malformed hand-templated body | Open the execution → failing HTTP node → response. All bodies must be the Expression `{{ JSON.stringify($json._llm_body) }}` with NO `temperature` |
| `Method Not Allowed` / 405 on an LLM node (Triage, Rescore, Cluster, Summarise) | The HTTP node's **Method** is still n8n's GET default — Anthropic's `/v1/messages` is POST-only | Open the node → set **Method: POST**. Applies to all four LLM HTTP nodes; the one that surfaces the error is just the first GET node the run reached |
| Ticker stories missing thesis wikilinks | `cfg.ticker_files` empty (Tickers command failed) or feedId prefix mismatch | Check Tickers stdout; wikilinks only attach to `tk-*` member stories by design |
| Same story re-briefed across runs/days | Story memory not reaching the cluster call (`_prior` empty in the 18a execution → PriorStories node broken), or the LLM repeat bar judging "new angle = advances" | Check `_prior` AND `_preFollowups` in ClusterPrep's output: the deterministic title-key guard (18a) should divert near-identical titles before the LLM; genuinely reworded repeats rely on the cluster prompt's bias-toward-repeat clause — sharpen it or widen `story_memory_days` |
| Registry prompt edit has no effect | `####` header misspelled, or a required token deleted (`{items}`; cluster also needs `{prior}`) — Plan silently reverted that stage to its card-5 `DEF_P` fallback, by design (a broken prompt must not kill the run) | §5.1 check 9's third grep must return 4; restore the exact header/token, then compare the prompt in the next execution's `_llm_body` against the registry text |
| Real development buried in ♻ Follow-up coverage | Repeat bar too loose | Tighten the "ADVANCES the story" clause in 18a's prompt; the ♻ section keeps the link either way — nothing is lost, only unsummarised |
| Run hangs at PreBody or Rejoin | A zero-item branch starved a Merge input (quiet day, all-bypass day, or zero body-eligible items) | Verify TriagePrep's guaranteed-batch line survived the paste and Always Output Data is ON for Admit + Final; if the card-17 chain itself never executed, n8n's v1 execution order should still settle the Merge — if yours doesn't, wire 16-true through a pass-through Code node with Always Output Data as the branch-keeper |
| Search loop (GDELT or Brave) stops partway; `done` never fires; channel contributes nothing | A zero-result or rate-limited query returned `[]` — the empty loop-back froze Loop Over Items (routine on weekends: past-day queries with no hits) | Always Output Data ON on GParse (8.3) and BParse (9.3); re-run. Then check GdeltLoop's `done` output is article-shaped — §5.5(g) |
| Normalize (and the whole chain after it) executed 3–5× in one run; multiple partial digests or a null-body 400 at Summarise | Multi-wire per-branch execution — each inbound delivery ran the node separately (confirmed on this instance 2026-07-20) | The 6b FeedTasks and 10b PreNormalize Merges must exist and carry ALL the listed wires; §5.4 map |
| PreNormalize shows thousands of items but Normalize outputs only the warnings sentinel; post-PreNormalize stages sprint in seconds | A leftover pre-10b direct wire into Normalize (Feeds error is the classic) fired it early on a near-empty delivery; the skeleton lineage then raced to the output stage — and a hard error there (e.g. WriteDigest path) kills the run before the real delivery processes | Normalize must have EXACTLY ONE inbound wire (from PreNormalize) — count them on the canvas, delete extras |
| Re-test run comes up empty end-to-end (Dedupe kills everything) | Card-12 static store still holds URLs from a run <`max_age_d+2` days ago | Rarely needed since the TTL rebuild (manual test runs may not even persist static data); to force-reset run a one-line Code node `$getWorkflowStaticData('global').seenUrls = {}` |
| Debugging needs fast iterations (each full run costs ~30 min in GDELT/Brave pacing) | — | Detach 4 wires: Route 2→GdeltLoop, GdeltLoop done→PreNormalize in3, Route 3→BraveLoop, BraveLoop done→PreNormalize in4 (a Merge ignores UNCONNECTED inputs; a connected-but-starved one stalls — delete at the Merge end too). Run = RSS+FMP+LLM ≈ 3–5 min. Re-attach per §5.4 when done |
| Brief warning list shows `normalize:dropped:N keys:…` | Normalize met items it could not classify (the sample names their field keys) or items whose link failed URL parsing (`badurl:` sample) | Match the sampled keys against card 11's branches; extend the link-field fallbacks or the classification test accordingly — the sample IS the diagnosis |
| `normalize:dropped:N badurl:` where the sampled URL is obviously VALID | The Code node used `new URL()` — the **URL global does not exist in this n8n Code sandbox** (confirmed live: it zeroed a 30k-item run); the constructor throws, try/catch nulls it, every URL "fails" | Card 11's canon/hostOf are pure regex for exactly this reason. Never introduce `new URL()` (or other browser/Node globals) into any W3 Code node — test sandbox globals with a one-line Code node before relying on them |
| `Referenced node doesn't exist` (may arrive wrapped in a nonsense `Cannot assign to read only property 'name'` error) on a node that never failed before | A `$('Name')` lookup in that node's code doesn't match any node title — the misnamed node sat harmless until this branch first carried items (e.g. Body1 fires only on the first run with body-eligible articles) | Rename the target node to the exact carded name (rename the node, not the code — n8n auto-updates expression refs); then audit all load-bearing names against the §5.3 convention note's warning |
| `Channels: … fmp 0` + `401 Invalid API KEY` entries in Source failures | The n8n FMP Query Auth credential holds a stale key — §5.1 check 7 tests the key in `.data/config.json` directly, which can pass while the credential differs | Update the n8n FMP credential with the `.data/config.json` key (they must match; config.json is the source of truth) |
| Nearly every story `*(headline only)*` but summaries ARE present and factual | Story mix dominated by Google News members (redirect URLs are body-exempt by design) — usually because GDELT/Brave/FMP contributed 0 that run (detached, broken credential, or rate-limited) | Check the `Channels:` line; restore the zero channels. Cross-channel redundancy is the designed fix — non-GN members bring the body text |
| `expects a number but we got '8'` (or similar) on a number-typed field fed by an expression | n8n strict type validation — expression results can arrive as strings even when the source value is numeric | Wrap the expression in `Number(…)` with a `\|\| <default>` fallback — card 8.4's Pace is the known case; any future registry-driven numeric field needs the same wrapper |
| Telegram send fails | >4,096 chars | Already capped at 3,900 (card 19) — check for multi-byte overflow, trim top list to 3 |
| Same story appears as two entries in ONE brief | Cluster model under-grouping — different-outlet headlines for one event judged as separate events | Three layers now: title keys strip `" - Publisher"` tails (Normalize/18a); SumPrep's deterministic Jaccard merge (≥0.5 token overlap) force-unions near-identical headlines regardless of the model's verdict; the LLM handles only genuinely-reworded cases — if THOSE still split chronically, escalate `cluster_model` to `claude-opus-4-8` (cell edit, last lever). To loosen/tighten the mechanical merge, adjust the 0.5 threshold in SumPrep |
| A feature behaves as if absent — its output field is missing entirely from a node's execution output (e.g. no `_prior`/`_preFollowups` on ClusterPrep, no `_ch`/`_stale` on the Normalize sentinel) | **Deployed-node drift**: the n8n node runs an older paste — doc edits never auto-sync | The missing field NAMES the stale node; re-paste that card from the doc. After any doc-side code round, resync every listed node — field presence in the next execution is the verification |
| Telegram/watchdog says "number of entries exceeded cap"; run dies at Dedupe | The old **Remove Duplicates** node is still in place — its count-capped history overflows from Google News URL churn (~10k throwaway keys/run) regardless of History Size (confirmed 2026-07-21: overflowed 50k in a few runs) | Replace it with the card-12 **Code "Dedupe"** node (TTL-bounded static store — bounded by time, cannot overflow). This is an architecture fix, not a setting: the count-based node has no eviction and will always overflow eventually |
| Scheduled workflow silently produces nothing | TWO distinct causes, disambiguate via Executions list: **no execution row** = never fired (Mac asleep — `pmset -c sleep 0`, §1.4) · **row exists, ran fast, wrote its state file but no user-facing output** = the workflow's own quiet-path gate went false (W5: nothing flagged to push — legitimate) OR an LLM node failed silently (On-Error-Continue) leaving nothing to flag — open the execution and check that node's method/output |
| Node's params show POST but the execution ran GET (or any param mismatch execution-vs-canvas) | The execution is a **historical snapshot** — it captured the config as it was AT RUN TIME. Either the POST edit postdates that run, or the workflow wasn't **saved** after the edit (unsaved canvas ≠ executed version). Rarer: a 3xx redirect from a malformed URL downgrades POST→GET | Re-save the workflow, manually Execute, and read the FRESH execution — it should show POST. If a freshly-saved POST still runs GET, check the URL for a trailing space/slash causing a redirect. Old executions never retroactively update |
| Unrelated items merged into one story | Cluster model over-grouping | Tighten the "when unsure, keep separate" line in the 18b prompt; check the story's Sources line to spot the intruder |
