...
**21 · Telegram** (from `XFanout`) — existing credential · **Chat ID** `1779654963` · **Text** → Expression → `{{ $json.text }}` · **Additional Fields → leave Parse Mode unset** (remove the field if present) · **Settings → On Error** `Continue (using regular output)`. Sends one message per input item (the fan-out) — Telegram's ~1 msg/s flood limit means ≤10 is safe; if 429s appear, lower `x_tg_max_msgs`.
- **Leave Parse Mode UNSET (mandatory):** n8n's Parse Mode dropdown offers only HTML / Markdown / MarkdownV2 — there is **no `None`**. If a Parse Mode field is present in Additional Fields (it may already be set to HTML), **remove the whole field** (hover the field row → click the ✕/trash). An *absent* `parse_mode` = plain text. The messages carry raw tweet text (full of `<` `>` `&` `*` `_` `[`); any parse mode makes Telegram try to interpret those as formatting and reject the message with `Bad Request: can't parse entities`.
...
| Telegram node 400 `can't parse entities: Can't find end of the entity` | The node's **Parse Mode** is set (HTML or Markdown) and the message has unbalanced formatting chars — raw tweet/summary text is full of `<` `>` `&` `*` `_` `[` `` ` `` | **Remove the Parse Mode field** from Additional Fields (n8n offers only HTML/Markdown/MarkdownV2 — no `None`; an *absent* `parse_mode` sends plain text). Also set the node **On Error: Continue**, or the 400 halts the run before the dashboard/state writes (why a failed push also produced no disk writes) |
