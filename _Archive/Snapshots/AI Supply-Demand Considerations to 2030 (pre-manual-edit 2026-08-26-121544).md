...
snapshot_batch: manual-edit-2026-08-26-121544
publish: true
date: 2026-08-25
...
The cost side closes the identity: cost per energised watt-year is flat (~$6.5 accounting on a 7-year accelerator and 25-year shell; ~$10.6 with the cost of capital charged), so the industry's cost of producing tokens grows with energised watts rather than with tokens: token-serving COGS ~$75B (2026) → ~$420B (2030), the full AI fleet ~$150B → ~$600B, against tokens ×40 and cost per token ÷7. Neocloud margins hold only if compute-dollar demand grows with energised watts; the cushion to the 15% floor is ~25% cumulative, and excess demand improves economics only on uncontracted capacity (§7).
...
Two readings of the coverage row. On these lives the industry is gross-profitable all-in throughout (1.1x rising to 1.4x), which is the version the hyperscalers will report; on the economic basis measured AI revenue does not pay the fleet's cost of capital before 2031 (0.7x rising to 0.9x), the remainder carried by unlabelled advertising, search and franchise value and by equity-funded training, which is the 5-7% spot ROIC of [[Macro & Technology/Sustainability of AI Capex]] restated from the cost side. The cost base is committed while the revenue is not, and the labs' ~70% inference gross margin is a mix statement, frontier pricing over a base of free-tier, internal and ads-funded serving that carries cost with no token revenue. One inversion follows: the bull case for serving margins is the slow power path, since scarcity holds COGS growth near +35% a year and revenue per watt high, and the fast BTM build the power complex is paid to deliver is what drives the cost base to $600B and margins down.

### The neocloud margin condition

The proposition to test: as long as dollar demand for tokens meets the COGS line, the neocloud industry maintains its current margin and grows at the supply rate, and if demand exceeds the line, economics improve. It holds in structure. Rent per watt-year is compute-dollar demand divided by energised watts; the neocloud's cost per watt-year is flat; so its spread holds if and only if compute dollars grow with watts, widens if they grow faster, and narrows if slower. Reference points per Rubin-class MW-year: famine $26-32M ($12-15/GPU-hr), the balance ladder $21-26M ($9.60-12), the 15% IRR floor $17.6M ($8.20), reported breakeven ~$7M (~$3.0-3.5 at a 7-year life). Today's contracted rent sits at ~2x economic cost and ~3.5x accounting cost.

| Compute-dollar demand vs the watts line, cumulative 2027-30 | Rent per MW-yr | Neocloud outcome |
|---|---|---|
| Faster | Above $26M, capped near $32-40M by the customer's value ceiling ($15-18/hr on FP8 throughput parity) | Famine rows, 22-30%+ ROIC; renewals above launch; NVIDIA recaptures part through system pricing (~40% headroom on the One Chart) and allocation |
| Equal | Flat at $21-26M | Balance ladder: 18-22% on new signings; the contracted book unchanged either way |
| ~25% slower (≈6pts a year below the watts CAGR) | $17.6M | 15% floor: profitable, no spread; re-rents land 35-40% |
| ~70% slower | ~$7M | Reported gross margin still positive on merchant capacity; economic value destruction from the row above |

It fails in four specifics. First, the variable is compute dollars, not token dollars: compute-dollar demand is token revenue × (1 − lab gross margin) + training budgets + hyperscaler internal transfer, and token revenue of $150-180B against $340-560B of compute dollars at today's transfer prices (24 GW at $14-23M per MW-year) funds well under half the watts, the rest being equity- and vendor-financed training and franchise defence. Token dollars meeting the serving line ($75B → $417B) is a statement about lab gross profit, not about rent; for the condition to hold on token dollars alone by 2030, token revenue must reach ~$1.0-1.3T, above the $850B bridge, because the training share of compute dollars is the tranche that stops when financing reprices. Second, 'current margin' is a famine margin, and holding it needs compute dollars at +45-60% a year through 2028 and +35% after, at or above the bridge with the token share of compute dollars rising; no industry has compounded revenue 50% a year for four years from a $150B base (cloud managed 35-40% at $50-100B), so maintaining current margins is the above-bridge case and the base case is the balance ladder with the famine premium decaying on new paper in 2028. Third, 'grow at the supply rate' is a share statement: neoclouds hold ~10% of energised watts (roughly 3-4 GW of ~30 GW at end-2026), and the contracted pipelines (NBIS 5 GW, CRWV 3.5 GW, IREN 4.5 GW, SpaceX's 10 GW target) imply a share rising toward 15-20% by 2028 only if energisation cadence holds and NVIDIA's allocation keeps steering silicon away from the CSPs; above-supply growth is the base case on pipelines, below-supply growth is the allocation-normalisation risk. Fourth, 'economics improve' lands on the merchant sliver (10-20%) and on new signings, not on the 80-90% contracted at fixed rates until 2030-31; it is capped by the value ceiling, partly clawed back by the layer owner and by the labs' cascade (§3), which squeezes rent before it raises posted prices; and the convexity is asymmetric, spot +150-300% on a 33% overhang against a harder crash on the way down, so the merchant upside is a famine artefact that decays as demand converges and the durable improvement is the renewal rate on the next contract. Underneath all four sits the counterparty point: three capex programmes underwrite most disclosed backlog, so the aggregate can meet the line while one anchor's 25-30% cut converts a slice of contracted paper into disputed paper; the financing channel decides the neocloud case before the token channel does.

Restated: neocloud margins hold if compute-dollar demand for neocloud capacity grows with neocloud energised watts; the cushion between today's contracted rent and the 15% floor is ~25% cumulative, ~6pts a year for four years; economics improve above that only on uncontracted capacity, capped and shared; and the token-revenue form of the condition is stricter than the revenue bridge because token dollars fund a minority of today's watts. The observables in §5 that read this directly are the rent per MW-year on new Rubin paper against energised-watt growth, and the blended realised price per million tokens at the labs against the serving cost curve.
