...
snapshot_batch: manual-edit-2026-08-26-122204
publish: true
date: 2026-08-25
...
The cost side closes the identity: cost per energised watt-year is flat (~$6.5 accounting on a 7-year accelerator and 25-year shell; ~$10.6 with the cost of capital charged), so the industry's cost of producing tokens grows with energised watts rather than with tokens: token-serving COGS ~$75B (2026) → ~$420B (2030), the full AI fleet ~$150B → ~$600B, against tokens ×40 and cost per token ÷7. Measured revenue covers ~0.7x of the fleet's economic cost, rising to ~0.9x by 2030 on the bridge: a monetisation gap (35-40% of watts paid for today, ~55% by 2030), not a price gap, so the industry balances as rents normalise rather than as the shortage deepens. Neocloud margins hold only if total compute dollars (token revenue plus training financing) grow with total watts, ×4 by 2030; the base case delivers ~×3-3.5, which lands new fleets at the 15% hurdle (§7).
...
| Monetised share of energised watts (inference share × paid share) | ~35-40% (inference ~50% of fleet; free-tier and internal a fifth to a third of that) | rising toward ~55% by 2030 = the balance path, with rents falling; flat = the economic gap is structural and the financing trigger moves forward (§7) |
...
Two readings of the coverage row. On these lives the industry is gross-profitable all-in throughout (1.1x rising to 1.4x), which is the version the hyperscalers will report; on the economic basis measured AI revenue does not pay the fleet's cost of capital before 2031 (0.7x rising to 0.9x), the remainder carried by unlabelled advertising, search and franchise value and by equity-funded training, which is the 5-7% spot ROIC of [[Macro & Technology/Sustainability of AI Capex]] restated from the cost side. One inversion follows: the bull case for serving margins is the slow power path, since scarcity holds COGS growth near +35% a year and revenue per watt high, and the fast BTM build the power complex is paid to deliver is what drives the cost base to $600B and margins down.

### Who earns the economic rate, and how the industry balances

The 0.7x is not a price problem. Prices sit at famine levels (§3); the shortfall is that most watts earn no measured revenue at all. Splitting the 2026 fleet by who pays for each slice:

| Slice, 2026 | Revenue per MW-yr | vs economic cost ($10.6M) | Who pays |
|---|---:|---:|---|
| Frontier-lab API serving, monetised MW | up to ~$100M (blended far lower) | 4-9x on owned watts; 2-4x net of rent | Enterprises and consumers |
| Neocloud contracted Rubin paper ($9.60-12/GPU-hr) | $21-26M | 2.0-2.5x (IRR 21-31% vs the 15% hurdle) | Labs with raised equity; hyperscalers with franchise cash |
| Hyperscaler external AI cloud (MSFT→OpenAI ~$14M; CSP $12-15M) | $12-15M | 1.1-1.4x | Labs, enterprises |
| Hyperscaler internal and free-tier serving | ~$0 measured (ad uplift unlabelled) | <1x | Ad and search franchise |
| Training watts, all owners (30-50% of the fleet) | $0 token revenue | 0x measured | Equity, vendor finance, operating cash flow |
| **Whole fleet, measured revenue** ($165B / 24 GW) | **$6.9M** | **0.7x** | |
| Whole fleet, revenue plus financing inflows ($340-400B) | $14-17M | 1.4-1.7x | |

The industry's economic return decomposes as the monetised share of watts × the multiple earned on monetised watts. In 2026 roughly 35-40% of watts are monetised (inference is half the fleet, and free-tier and internal serving take a fifth to a third of that) and they earn ~1.8x economic cost, so the fleet covers ~0.7x: the monetised slice earns about +$75B of economic profit, the unmonetised 60-65% of watts carry ~$150B of cost against nothing measured, net about −$75B against the $240B economic line (+$16B against the $149B accounting line). On the bridge the monetised share rises to ~55% by 2030 (inference 70% of the fleet, paid share ~80%) while the multiple on monetised watts eases from ~1.8x toward ~1.6x, so coverage rises to ~0.9x with net economic profit of about −$100B on a $950B line (+$255B on the accounting line), and the fleet reaches 1.0x around 2031-32 without any increase in scarcity.

That is the reconciliation with §3 and §4. The neocloud's 2x and the frontier lab's 4x are the famine multiple on the monetised slice, and it is paid largely by financial capital; the industry as a whole closes its gap not by a deeper shortage but by monetising a larger share of the fleet and by volume at falling prices ([G-14]), which is the same path along which rent per watt normalises to cost-plus. So the industry balances and the watt-owner's spread compresses in the same move: the installation-phase transfer from financiers to the owners of scarce inputs gives way to revenue-funded rent at the hurdle ([G-4]). A deeper shortage would raise the multiple on the monetised slice and leave the unmonetised watts unmonetised; it improves the neocloud's row, not the industry's.

### The neocloud margin condition

The proposition to test: as long as dollar demand for tokens meets the COGS line, the neocloud industry maintains its current margin and grows at the supply rate, and if demand exceeds the line, economics improve. It holds in structure and must be stated on all watts, because about half of neocloud capacity serves training. Rent per watt-year is total compute dollars (token revenue plus training and internal financing) divided by energised watts; the cost per watt-year is flat; so the spread holds if and only if total compute dollars grow with total watts (×4 on year-average watts, 24 → 96 GW, i.e. ~$350-400B → ~$1.4-1.6T by 2030), widens above that and narrows below. Reference points per Rubin-class MW-year: famine $26-32M ($12-15/GPU-hr), the balance ladder $21-26M ($9.60-12), the 15% IRR floor $17.6M ($8.20), reported breakeven ~$7M (~$3.0-3.5 at a 7-year life); today's contracted rent sits at ~2x economic cost and ~3.5x accounting cost, and the cushion to the floor is ~25%.

| 2030 case | Token revenue | Training and internal financing | Total compute $ | Rent per watt vs 2026 | New-fleet economics |
|---|---:|---:|---:|---:|---|
| Bridge, financing growing with operating cash flow (~12-15%/yr) | $850B | ~$300-375B | ~$1.15-1.25T | −15 to −25% | Contracted rents fall from $21-26M toward $17-21M: at or just above the 15% floor. §4 balance row |
| Bridge, financing flat at the ceiling | $850B | ~$200-235B | ~$1.05-1.1T | −30 to −35% | Below the $17.6M floor on new Rubin fleets. §4 glut row |
| Token revenue ×6-7, above the bridge | $1.0-1.2T | ~$300-375B | ~$1.4-1.6T | ~flat | Famine rows persist. §4 extended-famine row |

The base-case bridge thus consumes the whole cushion by 2030: neoclouds earn the hurdle on new fleets, not today's spread, and the split of the ×4 between token revenue (×5 on the bridge) and financing (×1.6 at the operating-cash-flow rate) is the number to watch. Three refinements to the proposition follow. 'Current margin' is a famine multiple paid by capital, so holding it is the above-bridge case, and no industry has compounded revenue 50% a year for four years from a $150B base (cloud managed 35-40% at $50-100B). 'Grows at the supply rate' is a share statement that belongs in the NBIS thesis (neoclouds hold ~10% of energised watts, with contracted pipelines implying 15-20% by 2028 if energisation and NVIDIA's allocation hold); at the industry level it reduces to the monetised share of watts not falling as the fleet quadruples. 'Economics improve' at the industry level is true on all watts, and the contract structure only decides who takes the surplus first (labs through posted prices and gross margin, as in 2025-26; watt-owners at renewal; then the silicon layer through the next generation's $/W, which is how a famine leaks back into the cost line, as the 2026-27 memory surge shows); the ceiling is the end customer's value per token rather than GPU-generation parity, the measured response is damped by take-or-pay (§3: +3-5% blended per 10% overhang), and the crash side is harder. Underneath sits the financing point: three capex programmes underwrite most disclosed backlog, so the aggregate can meet the line while one anchor's 25-30% cut converts a slice of contracted paper into disputed paper.

Restated: total compute dollars must grow ×4 by 2030 to keep today's margins; the base case delivers ~×3-3.5, so margins compress to the hurdle; better than that requires token revenue above the $850B bridge or financing compounding above operating cash flow. The observables in §5 that read this directly are the rent per MW-year on new Rubin paper against energised-watt growth, the blended realised price per million tokens at the labs against the serving cost curve, and the monetised share of energised watts.
...
- Generalist [G-4] · installation-to-deployment transfer — the neocloud's 2x and the lab's 4x are the famine multiple on the monetised 35-40% of watts, paid largely by financial capital; the fleet covers 0.7x of economic cost. Hypothesis: the industry balances via monetised share (→ ~55% by 2030) at falling rents, so watt-owner spreads compress in the same move that closes the industry's gap. Falsifier: coverage rising while contracted rents hold (a genuine value-layer regime, not a famine).
