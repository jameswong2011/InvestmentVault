...
2. **HBM Memory (SK Hynix / Samsung / Micron):** SK Hynix leads with ~60% HBM share, ~80% HBM3E yields, and first-to-market HBM4 status. Samsung's HBM4 yields remain ~50% (targeting 70% before mass production). A 16-Hi stack with 95% per-die yield drops to ~44% final yield — supply constraints sustain Nvidia's pricing power but also constrain production volume. Rubin Ultra's 8-Hi HBM4 despec ([[Research/2026-08-11 - MU NVDA Rubin Ultra 8-Hi HBM Despec - deep-dive]]) cuts local attach ~384GB→~192GB; bit-supply unlock is ≤1.63× layers, not a 2× stack-count windfall, and relocates the binding constraint from local HBM density to NVL576 interconnect + software (TileRT/Helix/Dynamo). HBM TAM projected $100B by 2028 (pulled forward 2 years).
