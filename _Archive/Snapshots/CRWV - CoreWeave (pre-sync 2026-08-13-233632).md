...
snapshot_batch: sync-2026-08-13-233632
publish: false
date: 2026-05-15
...
**6. Rubin's marginal fleet is not automatically value-destructive; the contract wrapper is the economic moat, shifting the bear case from launch capex to coverage and the 2028–2029 roll.** The corrected fleet model prices Rubin against a blended ~$6/GPU-hour Blackwell contract base and a $183K all-in invested cost per GPU. At a seven-year life, **1.5× pricing ($9/hour) produces $175K of lifetime EBIT and misses the $192K 15%-hurdle equivalent; 2.0× ($12/hour) produces $282K; 2.5× produces $389K; and 3.0× performance parity produces $496K**. The decisive asset is therefore not the rack but the five-year take-or-pay wrapper: at 2.0×, a contracted GPU earns ~$282K versus ~$122K on the merchant graduation curve, a **~$160K/GPU contract value**. This weakens the simple "Rubin capex doubles, therefore CRWV destroys value" bear case. It strengthens the thesis's existing failure mode instead: pre-memory-surge contracts below the ~1.6×/~$9.50 hurdle, incomplete coverage that pushes capacity onto the merchant curve from year one, or an age-six re-rent at the normalized-market 35–40% band rather than the modeled 55% blend. CRWV's base 120K-Rubin build remains pretax-positive across the sampled 1.5–3.0× range, but the build adds roughly $22B of debt; better fleet economics do not remove common equity's claim-stack risk. [[Research/2026-08-05 - NBIS CRWV - Rubin Fleet Economics - Rate Sensitivity Model - deep-dive]]
...
**8. What rate multiple and contract coverage does CRWV secure on Rubin tranches, and how much capacity is exposed to merchant pricing from day one?** The pre-tax fleet hurdle is roughly **$9.50/GPU-hour**, while the corrected deployment-rational floor is about **$10.60/hour** on a $6 Blackwell anchor; that floor falls below the hurdle if CRWV's true realized Blackwell base is below ~$5.40. The key disclosure is therefore the absolute Rubin rate, covered GPU/MW share, contract term, cancellation rights, power-cost pass-through, and age-six re-rent step—not headline backlog. *Answered by*: a dedicated Rubin contract or cohort disclosure; partial coverage or pricing below ~$9.50 validates the bear, while near-full coverage at ~$12+ and a later re-rent near the 55% blend would materially weaken it.
...
5. **Vera Rubin contracts clear the economic corridor, not merely the demand-pull test.** CRWV is among the first cloud providers deploying Rubin in H2 2026. At roughly **2.0× current Blackwell contract pricing (~$12/GPU-hour)**, the corrected model generates ~$282K lifetime EBIT per $183K invested GPU over seven years and clears the 15% hurdle; the five-year take-or-pay wrapper contributes roughly $160K/GPU versus a merchant fleet. The bull case requires near-full coverage inside the ~1.8–3.0× deployment corridor and a re-rent tail near the 55% blend—not just Rubin's headline performance gain.
...
- **Triggers + evidence status** — hypotheses tested, not verdicts:
	...
	- *Generalist [G-7] · ROIIC × runway / [G-13] · price-implied operating variable — Rubin rate and coverage, not fleet growth, are the hypothesis to test*: a seven-year contracted Rubin GPU clears the 15%-hurdle equivalent from ~2.0× pricing ($282K lifetime EBIT on $183K invested), while the same 2.0× GPU on the merchant graduation curve earns only ~$122K. The market can observe backlog and EBITDA while missing the ~$160K/GPU contract-wrapper value; the falsifier is a disclosed absolute rate and coverage share. This is an adversarial bull against the automatic-depreciation bear, not a verdict. (per [[Research/2026-08-05 - NBIS CRWV - Rubin Fleet Economics - Rate Sensitivity Model - deep-dive]])
...
- [[Research/2026-08-05 - NBIS CRWV - Rubin Fleet Economics - Rate Sensitivity Model - deep-dive]] — corrected fleet-economics model: contracted 2.0×+ Rubin pricing clears the hurdle; contract coverage and the 2028–2029 re-rent step, not launch capex alone, determine equity durability
